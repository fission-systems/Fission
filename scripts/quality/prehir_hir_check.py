...
static-pipeline diagnostic. The independent `fission-dir` product owns
P-code-rooted verification; this script is not a semantic equivalence proof.
It extracts a lightweight structural "signal" from each
side's rendered C-like text -- control-flow keywords, comparison/logic
operators, and integer constants, with identifier names stripped out (PreHIR
...
where HIR's signal is missing something PreHIR's had. A missing `if`/`return`/
...
(e.g. via `fission_cli decomp --addr <addr> --prehir <binary>`), not an
automatic failure.
...
--------
  ...
  python3 scripts/quality/prehir_hir_check.py check
  python3 scripts/quality/prehir_hir_check.py check --binaries control_flow_gcc_O0.exe
"""
...
def _strip_comment_lines(code: str) -> str:
    """Drop `// ...` lines (the "Function: ..." header and the "PreHIR
    (pre-structuring)" section marker `decomp --prehir` inserts) before
...
def structural_signal(code: str) -> Counter[str]:
    ...
    (variable/function names -- which legitimately differ between PreHIR and
...
def diff_signals(prehir_code: str, hir_code: str) -> tuple[Counter[str], Counter[str]]:
    ...
    prehir_sig = structural_signal(prehir_code)
    hir_sig = structural_signal(hir_code)
    return prehir_sig - hir_sig, hir_sig - prehir_sig
...
def decompile_all_with_prehir(
    ...
    if proc.returncode != 0:
        print(f"error: decomp --all --prehir failed for {binary.name}", file=sys.stderr)
...
def cmd_check(args: argparse.Namespace) -> int:
    ...
    for name in binaries:
        ...
        if not binary_path.exists():
            ...
            continue
        functions = decompile_all_with_prehir(cli, binary_path, args.limit, args.timeout_ms)
        for fn in functions:
            prehir_code = fn.get("code_prehir")
            hir_code = fn.get("code_hir")
            if not prehir_code or not hir_code:
                unsupported += 1
                continue
            checked += 1
            missing_in_hir, extra_in_hir = diff_signals(prehir_code, hir_code)
    ...
    print(
        f"checked {checked} functions ({unsupported} skipped: no code_prehir/code_hir)",
        file=sys.stderr,
    )
    ...
    if high:
        print(
            f"\nFAIL: {len(high)} function(s) with a high-severity PreHIR/HIR structural "
            "diff -- re-run the specific address with `decomp --addr <addr> --prehir` "
    ...
    print("\nPASS: no high-severity PreHIR/HIR structural diffs.")
    return 0
...
def build_parser() -> argparse.ArgumentParser:
    ...
    p_check = sub.add_parser("check", help="Compare PreHIR vs HIR structural signal across the corpus")
