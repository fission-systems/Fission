...
def run_command(cmd: list[str], cwd: Path, env: dict, timeout: int) -> str:
    try:
...
def compare_single(binary: Path, address: str, output_json: Path, timeout: int) -> dict:
    ...
    ghidra_cmd = [python_bin, str(script_dir / "pyghidra_decompile.py"), str(binary), address]
    fission_asm_cmd = fission_cmd + [str(binary), "--disasm-function", address]
    fission_decomp_cmd = fission_cmd + [str(binary), "--decomp", address]

    ghidra_full = run_command(ghidra_cmd, project_root, env, timeout)
    fission_asm = run_command(fission_asm_cmd, project_root, env, timeout)
    fission_decomp_raw = run_command(fission_decomp_cmd, project_root, env, timeout)
...
def generate_html_report(results: list[dict], output_dir: Path) -> None:
    ...
    html = [
        ...
        "<table><thead><tr><th>Address</th><th>Similarity</th><th>Ghidra</th><th>Fission</th></tr></thead><tbody>",
        *rows,
...
def main() -> int:
    ...
    if args.batch:
        output_dir = Path(args.output or (Path(__file__).resolve().parent / "result")).expanduser()
    ...
    if output_json is None:
        ...
        result_dir = Path(__file__).resolve().parent / "result" / f"{timestamp}_result"
