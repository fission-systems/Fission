...
- `[output_json]`: JSON 출력 파일 경로 (기본값: `scripts/json/comparison_result.json`)

**옵션:**
- `-m`: 배치 모드 활성화
- `-h`: HTML 리포트 생성 (배치 모드 전용)
- `-t N`: 타임아웃 설정 (초 단위, 기본: 300)
...
- ✅ 타임아웃 및 재시도 메커니즘
- ✅ 배치 모드: 여러 함수 일괄 처리
- ✅ HTML 리포트: 시각적 비교 대시보드
- ✅ jq를 통한 미리보기 지원

**결과 확인:**
```bash
# 전체 JSON 보기
cat scripts/json/comparison_result.json | jq .

# 특정 섹션만 보기
cat scripts/json/comparison_result.json | jq -r '.ghidra'
cat scripts/json/comparison_result.json | jq -r '.fission_decompilation'

# 메타데이터 확인
cat scripts/json/comparison_result.json | jq '.comparison_info'

# 메트릭 비교
cat scripts/json/comparison_result.json | jq '.comparison_info.metrics'

# 유사도 점수
cat scripts/json/comparison_result.json | jq '.comparison_info.similarity'
```
...
### JSON 출력 (compare_decompilers.sh)
...
```json
{
  "comparison_info": {
    ...
    "ghidra_chars": 123,
    "fission_asm_chars": 456,
    "fission_decomp_chars": 789
  },
  "ghidra": "Ghidra 디컴파일 결과 (C 코드)",
  "fission_disassembly": "Fission 어셈블리 출력",
...
**해결:**
```bash
# 최신 버전의 compare_decompilers.sh 사용
git pull origin main

# 수동 확인
cat scripts/json/comparison_result.json | jq empty
# 오류 없으면 JSON 형식 올바름
```
...
scripts/
├── README.md                   # 이 문서
├── build_decompiler.sh         # C++ 디컴파일러 빌드
├── compare_decompilers.sh      # Ghidra vs Fission 비교
├── pyghidra_decompile.py       # PyGhidra 활용 스크립트
├── ghidra_decompile.py         # 순수 Ghidra API 스크립트
├── test_fid.sh                 # FID 데이터베이스 테스트
└── json/                       # JSON 출력 디렉토리
    └── comparison_result.json  # 기본 출력 파일
```
...
for addr in $(seq 0x401000 0x401640 100); do
    ./scripts/compare_decompilers.sh test.exe $(printf "0x%X" $addr) \
...
- name: Compare Decompilers
  run: |
    ./scripts/compare_decompilers.sh test/binary 0x401000
    cat scripts/json/comparison_result.json | jq .
```

### 개발 팁
- **병렬 실행:** `compare_decompilers.sh`는 I/O 집약적이므로 여러 주소를 병렬로 처리 가능
