...
#### Example 3: `main` in `hello` (AArch64 Mach-O - High-fidelity String Recovery)
A simple C Hello World program (`hello.c`) compiled on macOS (AArch64 Apple Silicon) demonstrates Fission's robust Mach-O section parsing, library symbol matching (`printf`), and standard C format string literal recovery.
...
##### Comparative Analysis of `main` Outputs
- **String Constant Restoration**: Both Fission and Ghidra achieve perfect 100% equivalence in recovering the string literal `"Hello, World!\n"`. Fission uses its Mach-O `__cstring` section scanner and a specialized printer map to successfully inline global string literals directly into C expressions without any absolute numeric pointers or incorrect address-of (`&`) prefixes.
- **Register Storage Preservation**: Fission preserves the actual low-level architecture assignments and registers used for parameter storage (`local_10 = x1;` corresponding to register `X1` / `argv`) which are generated by the compiler but optimized away in Ghidra.
- **Data Flow and Return Control**: Fission accurately captures the return variable mapping (`local_c = 0; return local_c;`) representing how registers are initialized and returned according to the AArch64 calling convention.
