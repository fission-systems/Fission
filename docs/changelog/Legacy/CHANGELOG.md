...
# CLI one-shot surface reorganization

- `wave_type: refactor-only`
- `behavior_changed: yes`
- `release_path_changed: no`
- `env_gate: none`

Refactored `fission_cli` from a single flat one-shot flag surface into canonical subcommands: `info`, `list`, `disasm`, `decomp`, `strings`, and `inventory`. The internal execution path remains normalized through the existing `OneShotArgs` shape, while legacy flat invocations continue to work as deprecated shims that route into the same subcommand-driven dispatch path. Inventory/batch emitters now live under the operator-oriented `inventory` surface instead of the primary human-facing decomp path.
