...
|----|----------|------|-------|-------------------------|
| D1 | `normalize/cleanup/temp_var.rs` → `hoist_param_alias_copies_before_first_use` | Emergency pass | Still required (2026-07-10): synthetic clamp OK without hoist; **real m32 O2 clamp regresses** (edx use-before-def). Root: materialize does not emit dominating stack-param→reg alias for value reg before first compare | Keep hoist until materialize emits entry stack-param aliases in load order; see `docs/proposals/2026-07-10-retire-hoist-param-alias.md` (aborted) |
| D2 | `builder/materialize/loop_carried/binding.rs` → `loop_header_external_seed_binding_name_for_update` | ISA admission gate | ~~Early `return None` unless x86 CCs~~ **Fixed 2026-07-10:** admit via param slot / `is_primary_return_register` / register-space candidate only | Was: drop CC gate. Done in `docs/proposals/2026-07-10-loop-external-seed-cc-gate.md` |
