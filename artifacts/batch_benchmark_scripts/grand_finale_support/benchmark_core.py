    ...
    if isinstance(pair_rows, list):
        ...
        for row in ranked:
            ...
            if canonical in seen:
                continue
            seen.add(canonical)
            derived.append((canonical, "dynamic_low_similarity"))
            if len(derived) >= limit:
                break
    return derived
...
def write_summary_files(
    ...
    if row_targets:
        ...
        for row in row_targets.get("rows", []):
            if not row.get("present"):
                lines.append(
                    f"| {row.get('role', '')} | `{row.get('address', '')}` | n/a | no | n/a | n/a | n/a | n/a | n/a | n/a | n/a |"
                )
                continue
            lines.append(
                f"| {row.get('role', '')} | `{row.get('address', '')}` | "
                ...
                f"{int(row.get('fission_switch_emit_ready_failed_count', 0) or 0)} | "
                f"{int(row.get('fission_guarded_tail_rejected_side_entry_conflict_count', 0) or 0)} |"
            )
    ...
    lines.extend([
        ...
        f"- Fission switch emit-ready failed total: {summary['engines']['fission'].get('switch_emit_ready_failed_count', 0)}",
        f"- Fission guarded-tail candidate total: {summary['engines']['fission'].get('guarded_tail_candidate_count', 0)}",
        f"- Fission guarded-tail promoted total: {summary['engines']['fission'].get('guarded_tail_promoted_count', 0)}",
        f"- Fission guarded-tail side-entry conflict total: {summary['engines']['fission'].get('guarded_tail_rejected_side_entry_conflict_count', 0)}",
        f"- Fission guarded-tail alias-interleave conflict total: {summary['engines']['fission'].get('guarded_tail_rejected_alias_interleave_conflict_count', 0)}",
...
def write_corpus_summary_files(
    ...
    corpus = corpus_summary.get("corpus_summary", {})
    lines = [
        f"# Corpus Benchmark: {corpus_summary.get('manifest', {}).get('name', 'corpus')}",
        "",
        "## Corpus Summary",
        "",
    ...
    for item in corpus_summary.get("binaries", []):
        ...
        lines.append(
            f"| `{item.get('id', '')}` | `{item.get('role', '')}` | `{bool(eligibility.get('release_eligible', False))}` | {int(item.get('weight', 0) or 0)} | "
    ...
    for key, value in sorted(
        (corpus_summary.get("failure_family_distribution", {}) or {}).items()
    ):
    ...
    lines.extend(
        [
            "",
            "## Row Regression Reasons",
            "",
            "| Reason | Count |",
            "| --- | ---: |",
        ]
    )
    for key, value in sorted((corpus.get("row_regression_reasons", {}) or {}).items()):
        lines.append(f"| `{key}` | {int(value or 0)} |")

    lines.extend(
        [
            "",
            "## Canonical Rewrite Family Totals",
            "",
            "| Family | Total |",
            "| --- | ---: |",
        ]
    ...
    for key in (
        ...
        lines.append(
            f"| `{key}` | {int((corpus_summary.get('failure_family_distribution', {}) or {}).get(key, 0) or 0)} |"
        )

    lines.extend(["", "## Cross-Binary Degraded Watchlist", ""])
    cross_binary = corpus_summary.get("cross_binary_degraded_watchlist", [])
    if cross_binary:
        lines.extend(
            [
                "| Binary | Address | Function | Prev sim | Cur sim | Delta | Reasons |",
                "| --- | --- | --- | ---: | ---: | ---: | --- |",
            ]
        )
        for row in cross_binary[:15]:
            fn_name = row.get("fission_name") or row.get("pyghidra_name") or ""
            lines.append(
                f"| `{row.get('binary_id', '')}` | `{row.get('address', '')}` | `{fn_name}` | "
                f"{_safe_float(row.get('previous_normalized_similarity', 0.0), 0.0):.3f}% | "
                f"{_safe_float(row.get('current_normalized_similarity', 0.0), 0.0):.3f}% | "
                f"{_safe_float(row.get('normalized_similarity_delta', 0.0), 0.0):+.3f}pp | "
                f"{', '.join(row.get('reason_tags', [])) or 'none'} |"
            )
    else:
        lines.append("- No degraded rows recorded.")
...
def print_corpus_console_summary(
    ...
    for item in corpus_summary.get("binaries", []):
        eligibility = item.get("eligibility", {}) if isinstance(item.get("eligibility"), dict) else {}
        print(
            f"- {item.get('id')}: role={item.get('role')} "
            f"eligible={bool(eligibility.get('release_eligible', False))} "
            f"avg_norm={_safe_float(item.get('avg_normalized_similarity', 0.0), 0.0):.3f}% "
...
def main() -> int:
    ...
    if args.corpus_manifest is not None:
        ...
        output_dir = ensure_dir(
            ...
            if args.output_dir
            else DEFAULT_RESULTS_DIR / f"{Path(manifest['path']).stem}-latest"
        )
        baseline_summary_payload = (
