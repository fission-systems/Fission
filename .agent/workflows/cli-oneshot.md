...
```bash
./target/release/fission_cli <binary_path> --info
```
...
```bash
./target/release/fission_cli <binary_path> --funcs
```
...
```bash
./target/release/fission_cli <binary_path> --decomp <address>
```
...
```bash
./target/release/fission_cli <binary_path> --decomp <address> --verbose
```
...
```bash
./target/release/fission_cli tests/binaries/test_swift_bin --decomp 0x1000010a4
```
...
```bash
./target/release/fission_cli tests/binaries/test_go_bin --info
```
...
```bash
./target/release/fission_cli tests/binaries/test_objc_bin --funcs
```
...
```bash
cd crates/fission-tauri && npm run tauri dev
```
