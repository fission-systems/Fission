...
use crate::analysis::decomp::RecommendedDecompiler;
...
pub fn generate_pcode_graph(
    ...
    let mut decomp = match crate::analysis::decomp::ffi::DecompilerNative::new(&sla_dir) {
        Ok(d) => d,
        Err(e) => {
    ...
    let pcode_json = match decomp.get_pcode(addr) {
        Ok(json) => json,
        Err(e) => {
