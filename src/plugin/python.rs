...
use super::api::{BinaryInfo, PluginInfo, PluginType};
...
impl PythonRuntime {
    ...
    pub fn load_plugin(&mut self, path: &Path, plugin_id: &str) -> PyResult<PluginInfo> {
        ...
        Python::with_gil(|py| {
            ...
            let version = module
                .getattr("PLUGIN_VERSION")
                .and_then(|v| v.extract::<String>())
                .unwrap_or_else(|_| "0.1.0".to_string());

            let author = module
                .getattr("PLUGIN_AUTHOR")
                .and_then(|v| v.extract::<String>())
    ...
    pub fn dispatch_event(
        &self,
        event: &FissionEvent,
        event_bus: Option<&crate::core::events::EventBus>,
    ) {
        ...
        for plugin_id in self.modules.keys() {
            if let Err(e) = self.call_hook(plugin_id, hook_name, event) {
                let error_msg =
                    format!("Plugin '{}' hook '{}' error: {:?}", plugin_id, hook_name, e);
    ...
    fn event_to_pydict<'py>(
        ...
        match event {
            ...
            FissionEvent::DecompilationSuccess {
                address,
                function_name,
                code,
            } => {
