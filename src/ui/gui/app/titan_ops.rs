            ...
            if let Some(engine_lock) = &state.debug.titan_engine {
                if let Ok(engine) = engine_lock.read() {
                    ...
                    if !state.analysis.reconstructed_imports.is_empty() {
                        ...
                        if let Err(e) = engine.dump_and_fix("dumped_fixed.exe", &state.analysis.reconstructed_imports) {
                            state.log(format!("[Error] Dump & Fix failed: {}", e));
                        } else {
                            state.log("[✓] Process dumped and fixed to dumped_fixed.exe".to_string());
                        }
                    } else {
                    ...
                    if let Some(importer) = &mut engine.importer {
                        if let Err(e) = importer.update_modules() {
                            ...
                            if let Some(proc) = &engine.active_process {
                                ...
                                if let Ok(dos) = crate::debug_engine::pe::read_dos_header(proc.process_handle, base) {
                                    if let Ok(nt) = crate::debug_engine::pe::read_nt_headers64(proc.process_handle, base, dos.e_lfanew) {
                                        let import_rva = nt.OptionalHeader.DataDirectory[1].VirtualAddress;
                                        ...
                                        if import_rva != 0 {
                                            ...
                                            let iat_rva = nt.OptionalHeader.DataDirectory[12].VirtualAddress;
                                            ...
                                            if !use_heuristic {
                                                let target_rva = if iat_rva != 0 { iat_rva } else { import_rva };
                                                let target_size = if iat_size != 0 { iat_size } else { import_size };
                                                
                                                state.log(format!("[*] Scanning Standard IAT at RVA: {:X} (Size: {:X})", target_rva, target_size));
                                                
                                                if let Ok(res) = importer.reconstruct_iat(base + target_rva as u64, target_size as usize) {
                                                    if !res.is_empty() {
                                                        imports = res;
                                                        success = true;
                                                    } else {
                                            ...
                                            if use_heuristic {
                                                if let Ok(sections) = crate::debug_engine::pe::read_section_headers(proc.process_handle, base, dos.e_lfanew, nt.FileHeader.NumberOfSections) {
                                                    for section in sections {
                                                        let name = String::from_utf8_lossy(&section.Name).trim_matches(char::from(0)).to_string();
                                                        if name == ".rdata" || name == ".idata" || name == ".text" {
                                                            let start = base + section.VirtualAddress as u64;
                                                            let end = start + section.VirtualSize as u64;
                                                            state.log(format!("[*] Heuristic scanning section {} ({:X}-{:X})...", name, start, end));
                                                            
                                                            if let Ok((iat_start, iat_size)) = importer.find_iat_heuristic(start, end) {
                                                                state.log(format!("[*] Found potential IAT at {:X} (Size: {:X})", iat_start, iat_size));
                                                                if let Ok(res) = importer.reconstruct_iat(iat_start, iat_size) {
                                                                    if !res.is_empty() {
                                                                        imports.extend(res);
                                                                        success = true;
                                                                        break; 
                                            ...
                                            if success {
                                                state.log(format!("[✓] Reconstructed {} imports", imports.len()));
                                                state.analysis.reconstructed_imports = imports;
                                                state.ui.bottom_tab = crate::ui::gui::state::BottomTab::Imports;
                                            } else {
                                                state.log("[Error] IAT Reconstruction failed (No imports found)".to_string());
                                            }
                                        } else {
                                            state.log("[!] No Import Directory found in headers".to_string());
                                        }
                                    }
                                }
                            }
