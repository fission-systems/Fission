...
pub fn analyze_functions(state: &mut AppState) {
    ...
    if let Some(binary_arc) = binary_opt {
        ...
        state.log(format!("[✓] Found {} new internal functions ({} total)", 
...
pub fn navigate_to_address(
    ...
    let functions: Vec<FunctionInfo> = state.analysis.loaded_binary
        .as_ref()
    ...
    for func in &functions {
        ...
        if addr >= func.address && addr < func.address + range {
            state.log(format!("[*] Navigating to function: {} at 0x{:08X}", func.name, func.address));
