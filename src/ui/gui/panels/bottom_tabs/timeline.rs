...
pub fn render(ui: &mut egui::Ui, timeline: &mut Timeline) {
    ui.horizontal(|ui| {
        ...
        // Record button
        if is_recording {
            if ui.button(egui::RichText::new("⏹ Stop").color(catppuccin::RED)).clicked() {
                timeline.stop_recording();
            }
        } else if !is_replay {
            if ui.button(egui::RichText::new("⏺ Record").color(catppuccin::PEACH)).clicked() {
...
fn render_empty(ui: &mut egui::Ui, is_recording: bool) {
    ui.vertical_centered(|ui| {
        ...
        if is_recording {
            ...
            ui.label(egui::RichText::new("Step through the debugger to capture execution")
                .color(catppuccin::SUBTEXT0).small());
        } else {
            ui.label(egui::RichText::new("No recording available").color(catppuccin::OVERLAY0));
            ui.label(egui::RichText::new("Start recording while debugging to enable Time Travel")
                .color(catppuccin::SUBTEXT0).small());
        }
    });
}
...
fn render_replay_controls(ui: &mut egui::Ui, timeline: &mut Timeline) {
    ...
    ui.horizontal(|ui| {
        ...
        let response = ui.add(
            egui::Slider::new(&mut slider_pos, min_step as f64..=max_step as f64)
                .show_value(false)
        );
    ...
    if let Some(snap) = timeline.current_snapshot() {
        ...
        if !snap.memory_deltas.is_empty() {
            ui.separator();
            ui.label(egui::RichText::new(format!("Memory Changes ({})", snap.memory_deltas.len()))
                ...
                .show(ui, |ui| {
                    for delta in &snap.memory_deltas {
                        ui.horizontal(|ui| {
                            ...
                            ui.label(egui::RichText::new(format!("{:02x?}", &delta.old_value))
                                .color(catppuccin::RED).small());
                            ui.label("→");
                            ui.label(egui::RichText::new(format!("{:02x?}", &delta.new_value))
                                .color(catppuccin::GREEN).small());
                        });
                    }
                });
        }
