...
pub fn read_memory(process_handle: HANDLE, address: u64, size: usize) -> Result<Vec<u8>, String> {
    unsafe {
        ...
        if ReadProcessMemory(
            ...
            Err(format!("ReadProcessMemory failed: {:?}", std::io::Error::last_os_error()))
        }
    }
}
...
#[cfg(target_os = "windows")]
pub fn read_cstring(process_handle: HANDLE, address: u64, max_length: usize) -> Result<String, String> {
...
pub fn write_memory(process_handle: HANDLE, address: u64, data: &[u8]) -> Result<usize, String> {
    unsafe {
        ...
        if WriteProcessMemory(
            ...
            Err(format!("WriteProcessMemory failed: {:?}", std::io::Error::last_os_error()))
        }
    }
}
...
#[cfg(target_os = "windows")]
pub fn query_memory(process_handle: HANDLE, address: u64) -> Result<MEMORY_BASIC_INFORMATION, String> {
    unsafe {
        ...
        if VirtualQueryEx(
            ...
            Err(format!("VirtualQueryEx failed: {:?}", std::io::Error::last_os_error()))
        }
    }
}
