...
#[cfg(target_os = "windows")]
pub fn read_dos_header(process_handle: HANDLE, base_address: u64) -> Result<windows::Win32::System::SystemServices::IMAGE_DOS_HEADER, String> {
    ...
    let header: windows::Win32::System::SystemServices::IMAGE_DOS_HEADER = unsafe { std::ptr::read(data.as_ptr() as *const _) };
...
#[cfg(target_os = "windows")]
pub fn read_nt_headers64(process_handle: HANDLE, base_address: u64, e_lfanew: i32) -> Result<windows::Win32::System::SystemServices::IMAGE_NT_HEADERS64, String> {
    ...
    let header: windows::Win32::System::SystemServices::IMAGE_NT_HEADERS64 = unsafe { std::ptr::read(data.as_ptr() as *const _) };
...
pub fn read_section_headers(
    ...
    let nt_header_size = std::mem::size_of::<windows::Win32::System::SystemServices::IMAGE_NT_HEADERS64>();
    ...
    for i in 0..number_of_sections as usize {
        ...
        let section: windows::Win32::System::SystemServices::IMAGE_SECTION_HEADER = unsafe { 
            std::ptr::read(data[offset..].as_ptr() as *const _) 
        };
...
pub fn read_export_directory(
    ...
    let size = std::mem::size_of::<windows::Win32::System::SystemServices::IMAGE_EXPORT_DIRECTORY>();
    ...
    let dir: windows::Win32::System::SystemServices::IMAGE_EXPORT_DIRECTORY = unsafe { std::ptr::read(data.as_ptr() as *const _) };
    Ok(dir)
}
...
pub fn parse_exports(
    ...
    let func_rvas: &[u32] = unsafe { std::slice::from_raw_parts(func_data.as_ptr() as *const u32, num_funcs) };
    ...
    let name_rvas: &[u32] = unsafe { std::slice::from_raw_parts(name_data.as_ptr() as *const u32, num_names) };
    ...
    let ordinal_data = super::memory::read_memory(process_handle, ordinal_table_addr, ordinal_table_size)?;
    let name_ordinals: &[u16] = unsafe { std::slice::from_raw_parts(ordinal_data.as_ptr() as *const u16, num_names) };
    ...
    for i in 0..num_names {
        ...
        // Read name string
        if let Ok(name) = super::memory::read_cstring(process_handle, base_address + name_rva as u64, 256) {
    ...
    for i in 0..num_funcs {
        let rva = func_rvas[i];
        if rva == 0 { continue; } // Skip empty entries
