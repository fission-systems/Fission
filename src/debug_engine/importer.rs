...
impl ImportReconstructor {
            ...
            if EnumProcessModules(
                ...
                for i in 0..count {
                    ...
                    if !self.module_cache.contains_key(&base_addr) {
                        ...
                        if GetModuleInformation(self.process_handle, h_mod, &mut mod_info, std::mem::size_of::<MODULEINFO>() as u32).as_bool() {
                            self.module_cache.insert(base_addr, ModuleInfo {
                                name,
    ...
    #[cfg(target_os = "windows")]
    pub fn reconstruct_iat(&mut self, iat_start: u64, iat_size: usize) -> Result<Vec<ImportEntry>, String> {
    ...
    #[cfg(not(target_os = "windows"))]
    pub fn reconstruct_iat(&mut self, _iat_start: u64, _iat_size: usize) -> Result<Vec<ImportEntry>, String> {
        Ok(Vec::new())
    }

    #[cfg(target_os = "windows")]
    pub fn find_iat_heuristic(&mut self, start_addr: u64, end_addr: u64) -> Result<(u64, usize), String> {
        ...
        while current_addr < end_addr {
            ...
            if let Ok(data) = super::memory::read_memory(self.process_handle, current_addr, read_size) {
    ...
    #[cfg(target_os = "windows")]
    pub fn resolve_address(&mut self, address: u64) -> Result<(String, Option<String>, u32), String> {
                ...
                if let Ok(dos) = pe::read_dos_header(self.process_handle, base) {
                    if let Ok(nt) = pe::read_nt_headers64(self.process_handle, base, dos.e_lfanew) {
                        ...
                        if export_rva != 0 && export_size != 0 {
                            if let Ok(export_dir) = pe::read_export_directory(self.process_handle, base, export_rva) {
                                if let Ok(exports) = pe::parse_exports(self.process_handle, base, &export_dir) {
    ...
    #[cfg(not(target_os = "windows"))]
    pub fn resolve_address(&mut self, _address: u64) -> Result<(String, Option<String>, u32), String> {
        Err("Not supported".to_string())
    }
}
