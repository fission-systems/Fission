...
#[cfg(target_os = "windows")]
pub fn dump_process(process_handle: HANDLE, base_address: u64, output_path: &str) -> Result<(), String> {
    // 1. Read DOS Header
    let dos_header = super::pe::read_dos_header(process_handle, base_address)?;
    
    // 2. Read NT Headers
    let nt_headers = super::pe::read_nt_headers64(process_handle, base_address, dos_header.e_lfanew)?;
    ...
    for section in &sections {
        ...
        let read_size = if section.VirtualSize > 0 { section.VirtualSize as usize } else { raw_size };
        
        if let Ok(section_data) = super::memory::read_memory(process_handle, virtual_addr, read_size) {
...
#[cfg(target_os = "windows")]
pub fn rebuild_imports(file_path: &str, imports: &[super::importer::ImportEntry], original_base: u64) -> Result<(), String> {
    ...
    let dos_header = unsafe { &*(file_data.as_ptr() as *const windows::Win32::System::SystemServices::IMAGE_DOS_HEADER) };
    ...
    let nt_headers = unsafe { &mut *(file_data.as_mut_ptr().add(nt_offset) as *mut windows::Win32::System::SystemServices::IMAGE_NT_HEADERS64) };
    ...
    let mut modules: std::collections::HashMap<String, Vec<&super::importer::ImportEntry>> = std::collections::HashMap::new();
    for imp in imports {
        modules.entry(imp.module_name.clone()).or_default().push(imp);
    }
    ...
    // Calculate sizes
    let descriptor_size = std::mem::size_of::<windows::Win32::System::SystemServices::IMAGE_IMPORT_DESCRIPTOR>();
    ...
    for mod_name in &sorted_modules {
        ...
        // Align to 2 bytes
        if current_offset % 2 != 0 { current_offset += 1; }
        ...
        layout.insert(mod_name.clone(), ModuleLayout {
            name_offset,
    ...
    for mod_name in &sorted_modules {
        ...
        new_section_data[mod_layout.name_offset..mod_layout.name_offset + name_bytes.len()].copy_from_slice(name_bytes);
        ...
        for imp in *imps {
            ...
            if let Some(func_name) = &imp.function_name {
                ...
                new_section_data[entry_offset+2..entry_offset+2+fname_bytes.len()].copy_from_slice(fname_bytes);
                
                hint_name_offset_counter += 2 + fname_bytes.len() + 1;
                if hint_name_offset_counter % 2 != 0 { hint_name_offset_counter += 1; }
    ...
    let section_header_size = std::mem::size_of::<windows::Win32::System::SystemServices::IMAGE_SECTION_HEADER>();
    let section_table_offset = nt_offset + std::mem::size_of::<windows::Win32::System::SystemServices::IMAGE_NT_HEADERS64>();
    
    let last_section_offset = section_table_offset + (num_sections as usize - 1) * section_header_size;
    let last_section = unsafe { &*(file_data.as_ptr().add(last_section_offset) as *const windows::Win32::System::SystemServices::IMAGE_SECTION_HEADER) };
    ...
    for mod_name in &sorted_modules {
        ...
        for imp in *imps {
            if imp.function_name.is_some() {
                let ilt_pos = mod_layout.iat_offset + current_thunk_offset;
                let offset_val = u64::from_le_bytes(new_section_data[ilt_pos..ilt_pos+8].try_into().unwrap());
        ...
        // Create Descriptor
        let mut descriptor = windows::Win32::System::SystemServices::IMAGE_IMPORT_DESCRIPTOR::default();
        ...
        // Write Descriptor
        let desc_ptr = new_section_data.as_mut_ptr().add(descriptor_offset) as *mut windows::Win32::System::SystemServices::IMAGE_IMPORT_DESCRIPTOR;
        unsafe { *desc_ptr = descriptor; }
    ...
    let mut new_section_header = windows::Win32::System::SystemServices::IMAGE_SECTION_HEADER::default();
    ...
    unsafe {
        let header_ptr = file_data.as_mut_ptr().add(new_header_offset) as *mut windows::Win32::System::SystemServices::IMAGE_SECTION_HEADER;
        *header_ptr = new_section_header;
    }
    
    // 8. Update NT Headers
    nt_headers.FileHeader.NumberOfSections += 1;
    nt_headers.OptionalHeader.SizeOfImage = new_section_rva + ((current_offset as u32 + sect_align - 1) & !(sect_align - 1));
...
pub fn rebuild_imports(_file_path: &str, _imports: &[super::importer::ImportEntry], _original_base: u64) -> Result<(), String> {
