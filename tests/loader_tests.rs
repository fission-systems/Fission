...
fn test_load_self_executable() {
    ...
    #[cfg(target_os = "macos")]
    assert!(
        binary.format.starts_with("Mach-O"),
        "Expected Mach-O format on macOS, got: {}",
        binary.format
    );
    ...
    #[cfg(target_os = "linux")]
    assert!(
        binary.format.starts_with("ELF"),
        "Expected ELF format on Linux, got: {}",
        binary.format
    );
    ...
    assert!(
        binary.format.starts_with("PE"),
        "Expected PE format on Windows, got: {}",
        binary.format
    );
}
...
fn test_loaded_binary_builder() {
    use fission::analysis::loader::{FunctionInfo, LoadedBinaryBuilder};
...
fn test_section_parsing() {
    ...
    if let Ok(binary) = result {
        ...
        let has_code_section = binary
            .sections
            .iter()
            .any(|s| s.name.contains("text") || s.name.contains("TEXT") || s.is_executable);
...
fn test_architecture_detection() {
    ...
    if let Ok(binary) = result {
        ...
        assert!(
            !binary.arch_spec.is_empty(),
            "Architecture spec should not be empty"
        );
    }
}
