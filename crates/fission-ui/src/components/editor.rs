...
pub fn Editor() -> Element {
    ...
    // Build editor body
    // Import stubs get a dedicated info panel — no pseudocode decompile.
        ...
        match &active_tab {
            EditorTab::Pseudocode | EditorTab::Nir => {
                ...
                if let Some(code) = code_str {
                    // Import-stub content is rendered in a dedicated info panel,
                    // not with syntax colouring (it's a comment block, not code).
                    if is_import_stub {
                        rsx! {
                        ...
                        let (gutter, highlighted) = render_with_lines(&code);
                        rsx! {
                            // Thunk warning banner
                            if is_thunk {
            ...
            EditorTab::Hex => {
                let dump = {
                    let s = state.read();
                    s.binary.as_ref().map(|b| hex_dump(b.data.as_slice(), 4096))
                };
                if let Some(hex) = dump {
                    let (gutter, _) = render_with_lines(&hex);
                    // For hex we show line numbers but no syntax colouring
