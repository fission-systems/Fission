...
pub fn set_theme(ctx: &egui::Context, mode: crate::ui::gui::ThemeMode, dynamic_mode: bool) {
    match mode {
        crate::ui::gui::ThemeMode::Dark => apply_catppuccin_theme(ctx, dynamic_mode),
        crate::ui::gui::ThemeMode::Light => {
            let mut style = (*ctx.style()).clone();
            style.visuals = egui::Visuals::light();
            ctx.set_style(style);
        }
        crate::ui::gui::ThemeMode::System => {
            let mut style = (*ctx.style()).clone();
