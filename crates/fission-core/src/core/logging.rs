...
mod native {
    ...
    fn build_file_writer(path: &Path) -> io::Result<(NonBlocking, WorkerGuard)> {
        ...
        let file_name = path.file_name().ok_or_else(|| {
            io::Error::new(io::ErrorKind::InvalidInput, "log file path has no name")
        })?;
        let appender =
            tracing_appender::rolling::daily(dir, file_name.to_string_lossy().to_string());
...
impl LoggingOptions {
    ...
    pub fn with_mode(self, _mode: LoggingMode) -> Self {
        self
    }
...
#[cfg(target_arch = "wasm32")]
pub fn capture_span_trace() -> Option<String> {
    None
}
