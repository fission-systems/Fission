...
pub fn run_stage_proto_recovery_head(func: &mut PreHirFunction, _diag: bool, perf: bool) {
...
pub(super) fn cleanup_after_flag_recovery(func: &mut PreHirFunction) {
...
pub(super) fn body_contains_popcount_call_admitted(func: &PreHirFunction) -> bool {
...
pub(super) fn prune_after_elide_popcount(func: &mut PreHirFunction) -> bool {
...
/// Budget-gated (`CleanupPass`, matching the original `run_cleanup_block`).
pub(super) fn cleanup_after_entry_stack_scaffold(func: &mut PreHirFunction) {
    cleanup_func_stmt_list(func);
    defuse_dead_assignment_pass(func);
    prune_unused_temp_bindings(func);
    prune_unused_dead_local_bindings(func);
...
/// Budget-gated (`CleanupPass`, matching the original `run_cleanup_block`).
pub(super) fn cleanup_after_callee_save_prologue_epilogue(func: &mut PreHirFunction) {
    cleanup_func_stmt_list(func);
    defuse_dead_assignment_pass(func);
    prune_unused_temp_bindings(func);
    prune_unused_dead_local_bindings(func);
...
pub(super) fn constant_folding_body_pass(func: &mut PreHirFunction) -> bool {
    constant_folding_pass(&mut func.body)
}

/// Budget-gated (`CleanupPass`, matching the original `run_cleanup_block`).
pub(super) fn cleanup_after_constant_folding_init(func: &mut PreHirFunction) {
    cleanup_func_stmt_list(func);
...
/// see one definition.
pub(super) fn cleanup_after_constant_ptr_recovery(func: &mut PreHirFunction) {
    cleanup_func_stmt_list(func);
    prune_unused_temp_bindings(func);
    prune_unused_dead_local_bindings(func);
}
...
pub(super) fn cleanup_after_conditional_const(func: &mut PreHirFunction) {
    cleanup_func_stmt_list(func);
    constant_folding_pass(&mut func.body);
    eliminate_dead_local_clobber_assigns(func);
    eliminate_overwritten_assigns(func);
...
pub(super) fn cleanup_after_entry_param_promotion(func: &mut PreHirFunction) {
    cleanup_func_stmt_list(func);
    defuse_dead_assignment_pass(func);
    prune_unused_temp_bindings(func);
    prune_unused_dead_local_bindings(func);
...
pub(super) fn sccp_is_admitted(func: &PreHirFunction) -> bool {
...
pub(super) fn cleanup_after_sccp(func: &mut PreHirFunction) -> bool {
    let before = hir_shape(func);
    cleanup_func_stmt_list(func);
    hir_shape(func) != before
}
...
pub(super) fn cleanup_elim_8(func: &mut PreHirFunction) {
    cleanup_func_stmt_list(func);
    eliminate_dead_local_clobber_assigns(func);
    eliminate_overwritten_assigns(func);
    hoist_shared_select_casts(func);
...
pub(super) fn wide_dead_assignment_and_prune(func: &mut PreHirFunction) -> bool {
...
pub(super) fn defuse_after_cse_and_prune(func: &mut PreHirFunction) -> bool {
...
pub(super) fn cleanup_after_branch_prefix_hoist(func: &mut PreHirFunction) -> bool {
    let before = hir_shape(func);
    cleanup_func_stmt_list(func);
    hir_shape(func) != before
}
...
pub(super) fn defuse_after_branch_hoist_and_prune(func: &mut PreHirFunction) -> bool {
    let changed = defuse_dead_assignment_pass(func);
    prune_unused_temp_bindings(func);
    prune_unused_dead_local_bindings(func);
    changed
...
/// `cleanup_func_stmt_list` that used to open the `if` body.
pub(super) fn cleanup_after_gvn_join_hoist(func: &mut PreHirFunction) -> bool {
    let before = hir_shape(func);
    cleanup_func_stmt_list(func);
    hir_shape(func) != before
}
...
/// (already individually logged) plus the two bare prune calls that followed it.
pub(super) fn defuse_after_gvn_and_prune(func: &mut PreHirFunction) -> bool {
    let changed = defuse_dead_assignment_pass(func);
    prune_unused_temp_bindings(func);
    prune_unused_dead_local_bindings(func);
    changed
...
pub(super) fn cleanup_after_split_flow(func: &mut PreHirFunction) {
    cleanup_func_stmt_list(func);
    defuse_dead_assignment_pass(func);
    prune_unused_temp_bindings(func);
    prune_unused_dead_local_bindings(func);
...
pub fn run_stage_cast_elision(func: &mut PreHirFunction, diag: bool, perf: bool) {
...
pub(super) fn cleanup_after_subvar_trim(func: &mut PreHirFunction) -> bool {
    let before = hir_shape(func);
    cleanup_func_stmt_list(func);
    hir_shape(func) != before
}

pub(super) fn cleanup_after_entry_stack_scaffold_late(func: &mut PreHirFunction) {
    cleanup_func_stmt_list(func);
    defuse_dead_assignment_pass(func);
    prune_unused_temp_bindings(func);
    prune_unused_dead_local_bindings(func);
...
pub fn run_stage_heritage_value_recovery(func: &mut PreHirFunction, diag: bool, perf: bool) {
...
pub fn run_stage_merge(func: &mut PreHirFunction, diag: bool, perf: bool) {
    for round in 0..4 {
...
/// `cleanup_pass`).
pub(super) fn cleanup_after_single_pred_label_inline(func: &mut PreHirFunction) -> bool {
    let before = hir_shape(func);
    cleanup_func_stmt_list(func);
...
pub(super) fn cleanup_after_loop_condition_temps(func: &mut PreHirFunction) {
    cleanup_func_stmt_list(func);
    prune_unused_temp_bindings(func);
    prune_unused_dead_local_bindings(func);
}
...
pub(super) fn cleanup_prune_9(func: &mut PreHirFunction) {
    cleanup_func_stmt_list(func);
    prune_unused_temp_bindings(func);
    prune_unused_dead_local_bindings(func);
}
...
pub(super) fn cleanup_prune_10(func: &mut PreHirFunction) {
    cleanup_func_stmt_list(func);
    prune_unused_temp_bindings(func);
    prune_unused_dead_local_bindings(func);
}
...
pub(super) fn cleanup_standalone_15(func: &mut PreHirFunction) {
    cleanup_func_stmt_list(func);
}
...
/// individually logged in the original) plus the two bare, unlogged prune
/// calls that followed it, bundled under the same telemetry name.
pub(super) fn defuse_after_licm_and_prune(func: &mut PreHirFunction) -> bool {
    let changed = defuse_dead_assignment_pass(func);
    prune_unused_temp_bindings(func);
    prune_unused_dead_local_bindings(func);
    changed
...
pub fn run_stage_cleanup(func: &mut PreHirFunction, diag: bool, perf: bool) {
    let _ = run_pass_logged(
        func,
