...
fn collapse_common_exit_guard_chain_rejects_mixed_targets() {
    let original = vec![
        PreHirStmt::If {
            ...
            then_body: vec![PreHirStmt::Goto("exit_a".to_string())],
            else_body: Vec::new(),
        },
        PreHirStmt::If {
            cond: PreHirExpr::Var("b".to_string()),
            then_body: vec![PreHirStmt::Goto("exit_b".to_string())],
            else_body: Vec::new(),
        },
        PreHirStmt::Assign {
...
fn collapse_loop_exit_alias_return_rewrites_guarded_for_exit_copy() {
    let mut stmts = vec![
        PreHirStmt::If {
            ...
            then_body: vec![PreHirStmt::Goto("exit_zero".to_string())],
            else_body: Vec::new(),
        },
        PreHirStmt::Assign {
            lhs: PreHirLValue::Var("sum".to_string()),
    ...
    let mut stmts = vec![PreHirStmt::While {
        ...
        body: vec![
            ...
            PreHirStmt::If {
                ...
                then_body: vec![PreHirStmt::Goto("missing_continue".to_string())],
                else_body: Vec::new(),
            },
...
fn recover_guarded_loop_tail_accumulator_return_rewrites_stale_temp_return() {
    let mut stmts = vec![
        ...
        PreHirStmt::If {
            ...
            then_body: vec![PreHirStmt::Goto("loop_body".to_string())],
            else_body: Vec::new(),
        },
...
fn eliminate_dead_temp_assigns_keeps_used_reg_flag_artifact() {
    let mut stmts = vec![
        ...
        PreHirStmt::If {
            ...
            then_body: vec![PreHirStmt::Return(Some(PreHirExpr::Const(1, int(32))))],
            else_body: vec![PreHirStmt::Return(Some(PreHirExpr::Const(0, int(32))))],
        },
    ];
...
fn inline_single_use_temps_does_not_cross_label_boundary() {
    let mut stmts = vec![
        ...
        PreHirStmt::If {
            ...
            then_body: vec![PreHirStmt::Goto("loop_head".to_string())],
            else_body: Vec::new(),
        },
    ];
        ...
        body: vec![PreHirStmt::If {
            ...
            then_body: vec![PreHirStmt::Switch {
                ...
                cases: vec![PreHirSwitchCase {
                    values: vec![1],
                    body: vec![PreHirStmt::Return(Some(PreHirExpr::Const(10, int(32))))],
                }],
                default: Vec::new(),
            }],
            else_body: vec![PreHirStmt::Return(Some(PreHirExpr::Const(20, int(32))))],
        }],
        params: Vec::new(),
        locals: Vec::new(),
        return_type: NirType::Unknown,
