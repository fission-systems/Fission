//! A tree-walking interpreter for `fission_midend_core::ir`'s `DirStmt`/
//! `DirExpr` and `HirStmt`/`HirExpr` ASTs.
//!
//! This is the load-bearing piece that makes DIR-vs-HIR differential
//! verification possible without any real machine execution: DIR (what
//! `fission-pcode`'s builder emits natively, and normalize/structuring's own
//! internal passes read and rewrite) and HIR (structuring's real
//! `DirFunction -> HirFunction` conversion's output) are genuinely
//! independent types (see `fission_midend_core::ir::hir`'s module doc), so
//! this module defines two interpreters -- [`dir::interpret`] and
//! [`hir::interpret`] -- generated from one [`define_interp!`] macro
//! invocation each, so the interpretation *logic* can never silently drift
//! between the two even though the types are genuinely separate. Divergence
//! between DIR and HIR results is exactly what [`crate::diff::diff_dir_hir`]
//! is checking for -- if this file's own two copies disagreed on what
//! "the same" p-code-derived logic means, that would corrupt the very
...
//! `fission-emulator` for the same pattern): `Load`/`PtrOffset`/`Index`/
...
//! model in Phase 1 -- verification is scoped to pure/arithmetic functions.
//! A function that touches any of these bails out of verification with a
//! clear reason rather than being silently skipped or falsely reported
//! equivalent.
//!
...
/// `UnaryOp`/`Binding`/`Function` family (either all `Dir*` or all `Hir*`,
/// from `fission_midend_core::ir`). See this file's module doc for why this
/// is a macro instantiated twice rather than one generic/trait-based
/// implementation or two independently hand-written copies.
    ...
    (
        ...
        pub mod $modname {
            ...
            use fission_midend_core::ir::{
                $BinaryOp, $Binding, $Expr, $Function, $LValue, $Stmt, $UnaryOp, NirType,
            };
            ...
            fn width_of(ty: &NirType) -> u32 {
                match ty {
                    ...
                    NirType::Ptr(_) => 64,
                    // Not reachable for any Phase-1-supported expression
            ...
            /// -- every Phase-1-supported expr variant carries its own
            /// result type explicitly (`Const(_, ty)`, `Cast{ty,..}`,
            /// `Unary{ty,..}`, `Binary{ty,..}`, `Select{ty,..}`) except
            /// `Var`, which is looked up in the environment's declared
            /// param/local types.
            ...
            fn eval_expr(expr: &$Expr, env: &Env) -> Result<Value> {
                match expr {
                    ...
                    other => bail!(
                        "interp: unsupported expr {other:?} -- no memory/call model in Phase 1, \
                         see module docs"
                    ),
                }
            }
            ...
            /// Phase 1 evaluates -- real x86 comparisons almost never
            /// survive as a plain binary-op comparison; they go through
            /// this flag-decomposition machinery instead (`of =
            /// __sborrow(a,b); ... if (zf || xVar) ...`), so without
            /// recognizing these specific four, Phase 1 couldn't verify
            /// almost any real x86 function with a comparison in it --
            /// confirmed by running this crate's real-binary test before
            /// this was added (see `tests/real_binary_end_to_end.rs`).
            /// Any other `Call` target still bails -- a small, fixed,
            /// well-known whitelist of *pure* intrinsics, not general
            ...
            /// definition: OF after a signed subtract / `cmp`) -- this is
            /// the one this crate's real end-to-end test actually hit
            /// (`__sborrow` guarding an `if (a > b)`-style comparison's
            /// flag recovery).
            ...
            fn exec_stmt(stmt: &$Stmt, env: &mut Env) -> Result<Flow> {
                match stmt {
                    $Stmt::Assign { lhs, rhs } => {
                        ...
                        // (confirmed empirically: this crate's real
                        // end-to-end test fixture assigns `local_4` in
                        // both `if` arms with no declaration for it at
                        // all). Rather than bailing on every such case,
                        // infer the type from the RHS and register it as
                        // a fresh variable the first time it's assigned.
                    ...
                    $Stmt::VaStart { .. } => {
                        bail!("interp: variadic functions are not supported in Phase 1")
                    }
            ...
            /// `locals`/`body` -- the usual entry point (see
            /// [`interpret`] for the lower-level, explicit-parts version
            /// `crate::diff::diff_dir_hir` uses to let DIR and HIR share
...
pub use dir::interpret as interpret_dir;
