...
use crate::ir::{DecompFacts, NirBuildStats, PreHirFunction};
...
pub fn norm_trace_hash(func: &PreHirFunction) -> u64 {
    use std::hash::{Hash, Hasher};
    let mut hasher = std::collections::hash_map::DefaultHasher::new();
    // Deliberately excludes callee_observed_max_arity/callee_summaries and
    // other bookkeeping fields -- hashing the *whole* PreHirFunction produced
    ...
    format!(
        "{:?}",
        (&func.body, &func.locals, &func.params, &func.return_type)
    )
    .hash(&mut hasher);
    hasher.finish()
}
...
pub struct FnPass {
    ...
    pub f: fn(&mut PreHirFunction) -> bool,
}
