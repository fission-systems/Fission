...
use fission_midend_core::ir::{
    CallSummary, HirFunction, HirStmt, NirBinding, NirBindingOrigin, NirType,
};
use indexmap::IndexMap;

use crate::ir::{PreHirExpr, PreHirStmt, prehir_expr_to_hir_expr};

/// The PreHIR-side counterpart to [`NirBinding`] -- identical except
/// `initializer`, which embeds a real AST expression and therefore must be
/// `PreHirExpr`-typed on the pre-structuring side. Everything else
/// (`name`/`ty`/`surface_type_name`/`origin`) is genuinely shared,
/// type-agnostic metadata.
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct PreHirBinding {
    pub name: String,
    pub ty: NirType,
    pub surface_type_name: Option<String>,
    pub origin: Option<NirBindingOrigin>,
    pub initializer: Option<PreHirExpr>,
}
...
impl Default for PreHirFunction {
    fn default() -> Self {
        Self {
            name: String::new(),
            params: Vec::new(),
...
impl PreHirFunction {
    ...
    /// `PreHirFunction -> HirFunction` boundary (`prehir_stmts_to_hir_stmts`,
    ...
    /// need converting because `PreHirBinding::initializer` is `PreHirExpr`-typed).
    pub fn into_hir_function(self, body: Vec<HirStmt>) -> HirFunction {
        HirFunction {
            ...
            float_param_offsets: self.float_param_offsets,
            float_shares_int_slots: self.float_shares_int_slots,
...
fn dir_binding_to_nir_binding(binding: PreHirBinding) -> NirBinding {
    NirBinding {
        name: binding.name,
        ty: binding.ty,
        surface_type_name: binding.surface_type_name,
        origin: binding.origin,
        initializer: binding.initializer.map(prehir_expr_to_hir_expr),
    }
}
