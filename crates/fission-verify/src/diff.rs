//! DIR-vs-HIR differential verification (concrete tier, no ground truth).
//!
//! Runs the same concrete argument tuples through [`crate::eval::interpret_dir`]/
//! [`crate::eval::interpret_hir`] against a function's pre-structuring form
//! (DIR) and post-structuring form (HIR) and reports any input where the two
//! disagree -- a real structuring bug, with a concrete repro input, not a
//! hand-diffed hunch.
//!
//! This tier alone does **not** rule out a bug shared by both DIR and HIR
...
use crate::eval::{interpret_dir, interpret_hir};
