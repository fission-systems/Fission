//! A tree-walking interpreter for `DirStmt`/`DirExpr` (from
//! `fission-midend-dir`) and `HirStmt`/`HirExpr` (from `fission-midend-core`).
//!
//! This is the concrete-tier backend used by both [`crate::diff`] (DIR-vs-HIR
//! diffing, no ground truth) and [`crate::ground_truth`] (DIR/HIR-vs-real-
//! machine-code comparison). Two interpreters, [`dir::interpret`] and
//! [`hir::interpret`], generated from one [`define_interp!`] macro
//! invocation each, so the interpretation *logic* can never silently drift
//! between the two even though `DirStmt`/`HirStmt` are genuinely separate
//! types (see `fission_midend_core::ir::hir`'s module doc). Divergence
//! between DIR and HIR results is exactly what this crate exists to check
...
/// `fission_midend_dir`, or all `Hir*` plus the shared `NirBinding`,
