...
pub mod engine;
pub mod facts;
pub mod legacy_postprocess;

/// Compatibility shim — prefer [`legacy_postprocess`] for new code.
pub mod postprocess {
    pub use super::legacy_postprocess::*;
}
pub mod recovery;
pub mod render;
pub mod request;
...
pub use legacy_postprocess::{PostProcessor, RustPostProcessOptions};
pub use request::{
    DecompileRequest, DecompileResult, PostprocessPolicy, decompile_prebuilt_pcode,
};
...
mod tests {
    ...
    #[test]
    fn prebuilt_pcode_legacy_text_postprocess_not_applied() {
        ...
        assert!(!result.postprocess_applied);
        assert_eq!(
            result.postprocess_policy,
