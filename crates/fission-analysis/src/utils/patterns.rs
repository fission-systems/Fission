...
pub static SIGN_BIT: Lazy<Regex> =
    Lazy::new(|| Regex::new(r"\(int\)\s*(\w+)\s*>>\s*0x1[fF]").unwrap_or_else(|e| panic!("regex should compile: {}", e)));
...
pub static CONCAT44_ZERO: Lazy<Regex> =
    Lazy::new(|| Regex::new(r"CONCAT44\s*\(\s*0\s*,\s*(?P<lo>[^)]+?)\s*\)").unwrap_or_else(|e| panic!("regex should compile: {}", e)));

/// CONCAT with input register pattern (first position)
pub static CONCAT_INPUT_FIRST: Lazy<Regex> = Lazy::new(|| {
    Regex::new(r"CONCAT\d+\s*\(\s*(?:(?:\([^)]*\)\s*)?in_\w+\s*,\s*(?P<real1>[^,)]+))\s*\)").unwrap_or_else(|e| panic!("regex should compile: {}", e))
});

/// CONCAT with input register pattern (second position)
pub static CONCAT_INPUT_SECOND: Lazy<Regex> = Lazy::new(|| {
    Regex::new(r"CONCAT\d+\s*\(\s*(?P<real2>[^,)]+?)\s*,\s*(?:\([^)]*\)\s*)?in_\w+\s*\)").unwrap_or_else(|e| panic!("regex should compile: {}", e))
});
...
pub static UNSIGNED_RSHIFT: Lazy<Regex> =
    Lazy::new(|| Regex::new(r"\(uint\)\s*(?P<val>\w+)\s*>>\s*(?P<sh>\d+)\b").unwrap_or_else(|e| panic!("regex should compile: {}", e)));

/// Left shift pattern
pub static LEFT_SHIFT: Lazy<Regex> =
    Lazy::new(|| Regex::new(r"\b(?P<val>\w+)\s*<<\s*(?P<sh>[1-9]\d*)\b").unwrap_or_else(|e| panic!("regex should compile: {}", e)));

/// Bitwise AND mask pattern
pub static BITWISE_AND_MASK: Lazy<Regex> =
    Lazy::new(|| Regex::new(r"\b(?P<val>\w+)\s*&\s*(?P<mask>0x[0-9a-fA-F]+)\b").unwrap_or_else(|e| panic!("regex should compile: {}", e)));
...
pub static XOR_SIGN_BIT: Lazy<Regex> = Lazy::new(|| {
    Regex::new(r"\b(?P<val>\w+)\s*\^\s*(?:0x80000000|-2147483648)\b").unwrap_or_else(|e| panic!("regex should compile: {}", e))
});

/// Multiplication context detector
pub static MULT_CONTEXT: Lazy<Regex> =
    Lazy::new(|| Regex::new(r"\*\s*(0[xX][0-9a-fA-F]+|[1-9][0-9]*)").unwrap_or_else(|e| panic!("regex should compile: {}", e)));

/// Bitwise operation context detector
pub static BITWISE_CTX: Lazy<Regex> = Lazy::new(|| Regex::new(r">>|<<|\^|\b&\b|\|").unwrap_or_else(|e| panic!("regex should compile: {}", e)));
...
pub static PTR_OFFSET: Lazy<Regex> =
    Lazy::new(|| Regex::new(r"\*\s*\(\s*(\w+)\s*\+\s*(0x[0-9a-fA-F]+|\d+)\s*\)").unwrap_or_else(|e| panic!("regex should compile: {}", e)));
...
pub static ARRAY_INDEX: Lazy<Regex> =
    Lazy::new(|| Regex::new(r"(\w+)\[\s*(0x[0-9a-fA-F]+)\s*\]").unwrap_or_else(|e| panic!("regex should compile: {}", e)));

/// Field offset pattern: var._N_M_
pub static FIELD_OFFSET: Lazy<Regex> =
    Lazy::new(|| Regex::new(r"(\w+)\._([\d]+)_([\d]+)_").unwrap_or_else(|e| panic!("regex should compile: {}", e)));

/// Double pointer deref pattern: (**(*ptr + offset))
pub static DOUBLE_PTR_DEREF: Lazy<Regex> = Lazy::new(|| {
    Regex::new(r"\(\*\*\([\w\s\*]+\)\(\*(\w+)\s*\+\s*(0x[0-9a-fA-F]+)\)\)").unwrap_or_else(|e| panic!("regex should compile: {}", e))
});

/// XMM register field pattern: axVarN._8_8_
pub static XMM_FIELD: Lazy<Regex> = Lazy::new(|| Regex::new(r"(axVar\d+)\._(8)_(8)_").unwrap_or_else(|e| panic!("regex should compile: {}", e)));

/// Swift/Rust mangled name pattern
pub static MANGLED_NAME: Lazy<Regex> = Lazy::new(|| {
    Regex::new(r"(_\$s[a-zA-Z0-9_\$]+|__T[a-zA-Z0-9_\$]+|_T[a-zA-Z0-9_\$]+)").unwrap_or_else(|e| panic!("regex should compile: {}", e))
});

/// For loop initialization pattern
pub static FOR_INIT: Lazy<Regex> = Lazy::new(|| Regex::new(r"for\s*\(\s*(\w+)\s*=").unwrap_or_else(|e| panic!("regex should compile: {}", e)));

/// Generic variable pattern
pub static GENERIC_VAR: Lazy<Regex> =
    Lazy::new(|| Regex::new(r"^(local_\w+|[a-z]Var\d+)$").unwrap_or_else(|e| panic!("regex should compile: {}", e)));

/// Identifier pattern
pub static IDENTIFIER: Lazy<Regex> = Lazy::new(|| Regex::new(r"\b([a-zA-Z_]\w*)\b").unwrap_or_else(|e| panic!("regex should compile: {}", e)));

/// Main function entry pattern
pub static MAIN_FUNC: Lazy<Regex> = Lazy::new(|| Regex::new(r"\bmain\s*\(").unwrap_or_else(|e| panic!("regex should compile: {}", e)));

/// Return statement pattern
pub static RETURN_STMT: Lazy<Regex> =
    Lazy::new(|| Regex::new(r"\breturn\s+(\w+)\s*;").unwrap_or_else(|e| panic!("regex should compile: {}", e)));

/// Function call assignment pattern
pub static FUNC_CALL_ASSIGN: Lazy<Regex> = Lazy::new(|| {
    Regex::new(r"\b(local_\w+|[a-z]Var\d+)\s*=\s*(?:\([^)]*\)\s*)?(\w+)\s*\(").unwrap_or_else(|e| panic!("regex should compile: {}", e))
});

/// Undefined type declaration pattern
pub static UNDEF_TYPE_DECL: Lazy<Regex> =
    Lazy::new(|| Regex::new(r"\b(undefined\d*)\b\s+(\w+\s*\()").unwrap_or_else(|e| panic!("regex should compile: {}", e)));
...
pub static EMPTY_ELSE: Lazy<Regex> = Lazy::new(|| Regex::new(r"\}\s*else\s*\{\s*\}").unwrap_or_else(|e| panic!("regex should compile: {}", e)));
...
pub static IF_NOT_PATTERN: Lazy<Regex> = Lazy::new(|| {
    Regex::new(r"^(\s*)if\s*\(\s*!(\w+)\s*\)\s*\{\s*(return\s+[^;]+;)\s*\}").unwrap_or_else(|e| panic!("regex should compile: {}", e))
});

/// If comparison pattern
pub static IF_COMPARISON: Lazy<Regex> = Lazy::new(|| {
    Regex::new(r"^\s*if\s*\(\s*(\w+)\s*[<>]=?\s*(?:0[xX][0-9a-fA-F]+|\d+)\s*\)\s*\{").unwrap_or_else(|e| panic!("regex should compile: {}", e))
});

/// Return statement (line level)
pub static RETURN_LINE: Lazy<Regex> =
    Lazy::new(|| Regex::new(r"^\s*(return\s+[^;]+;)\s*$").unwrap_or_else(|e| panic!("regex should compile: {}", e)));

/// Close brace only pattern
pub static CLOSE_BRACE: Lazy<Regex> = Lazy::new(|| Regex::new(r"^\s*\}\s*$").unwrap_or_else(|e| panic!("regex should compile: {}", e)));

/// If-else combined pattern
pub static IF_ELSE_COMBINED: Lazy<Regex> = Lazy::new(|| {
    Regex::new(r"^(\s*)if\s*\(\s*(\w+)\s*(?:==|!=|<|<=|>|>=)\s*([^)]+)\s*\)\s*\{\s*$").unwrap_or_else(|e| panic!("regex should compile: {}", e))
});

/// Else block start pattern
pub static ELSE_START: Lazy<Regex> =
    Lazy::new(|| Regex::new(r"^\s*(?:\}\s*)?else\s*\{\s*$").unwrap_or_else(|e| panic!("regex should compile: {}", e)));

/// Assignment statement pattern
pub static ASSIGNMENT: Lazy<Regex> =
    Lazy::new(|| Regex::new(r"^\s*(\w+)\s*=\s*([^;]+);\s*$").unwrap_or_else(|e| panic!("regex should compile: {}", e)));
...
pub static WHILE_TRUE: Lazy<Regex> =
    Lazy::new(|| Regex::new(r"^(\s*)while\s*\(\s*(?:true|1)\s*\)\s*\{\s*$").unwrap_or_else(|e| panic!("regex should compile: {}", e)));

/// If break pattern
pub static IF_BREAK: Lazy<Regex> = Lazy::new(|| {
    Regex::new(r"^\s*if\s*\(\s*(.+?)\s*\)\s*\{?\s*break\s*;\s*\}?\s*$").unwrap_or_else(|e| panic!("regex should compile: {}", e))
});
...
pub static COMPOUND_ASSIGN: Lazy<Regex> =
    Lazy::new(|| Regex::new(r"^\s*(\w+)\s*(\+=|-=)\s*(.+?)\s*;\s*$").unwrap_or_else(|e| panic!("regex should compile: {}", e)));
...
pub static WHILE_TRUE_ML: Lazy<Regex> =
    Lazy::new(|| Regex::new(r"(?m)^(\s*)while\s*\(\s*(?:true|1)\s*\)\s*\{").unwrap_or_else(|e| panic!("regex should compile: {}", e)));
...
/// Do-while pattern
pub static DO_OPEN: Lazy<Regex> = Lazy::new(|| Regex::new(r"^(\s*)do\s*\{\s*$").unwrap_or_else(|e| panic!("regex should compile: {}", e)));

/// Do-while close pattern
pub static DO_WHILE_CLOSE: Lazy<Regex> = Lazy::new(|| {
    Regex::new(r"^(\s*)\}\s*while\s*\(\s*(\w+)\s*(!=|<|<=|>|>=)\s*([^)]+)\)\s*;\s*$").unwrap_or_else(|e| panic!("regex should compile: {}", e))
});

/// Single identifier pattern
pub static SINGLE_IDENT: Lazy<Regex> = Lazy::new(|| Regex::new(r"^\w+$").unwrap_or_else(|e| panic!("regex should compile: {}", e)));

/// Cast to simple variable pattern
pub static CAST_VAR: Lazy<Regex> =
    Lazy::new(|| Regex::new(r"^\s*(?:\(\s*\w+\s*\)\s*)?(\w+)\s*$").unwrap_or_else(|e| panic!("regex should compile: {}", e)));

/// Increment (++) pattern
pub static INC_PP: Lazy<Regex> = Lazy::new(|| Regex::new(r"^\s*(\w+)\s*\+\+\s*;\s*$").unwrap_or_else(|e| panic!("regex should compile: {}", e)));
...
pub static ADD_PATTERN: Lazy<Regex> =
    Lazy::new(|| Regex::new(r"^\s*(\w+)\s*=\s*(\w+)\s*\+\s*(.+?)\s*;\s*$").unwrap_or_else(|e| panic!("regex should compile: {}", e)));
...
pub static INLINE_ASM: Lazy<Regex> = Lazy::new(|| {
    Regex::new(r"(?s)__asm\s*\([^\)]*\)|asm\s*\([^\)]*\)|__asm__\s*\([^\)]*\)").unwrap_or_else(|e| panic!("regex should compile: {}", e))
});

/// Printf-like debug pattern
pub static PRINTF_DEBUG: Lazy<Regex> = Lazy::new(|| {
    Regex::new(r"(?s)(printf|fprintf|NSLog|os_log|android_log_print)\s*\([^\{]*\);?").unwrap_or_else(|e| panic!("regex should compile: {}", e))
});

/// Go panic pattern
pub static GO_PANIC: Lazy<Regex> = Lazy::new(|| {
    Regex::new(r"(?s)if\s*\([^\{]*\)\s*\{\s*runtime\.gopanic\([^\{]*\);?\s*\}").unwrap_or_else(|e| panic!("regex should compile: {}", e))
});

/// Malloc/calloc/realloc pattern
pub static MALLOC_PATTERN: Lazy<Regex> = Lazy::new(|| {
    Regex::new(r"(\w+)\s*=\s*((?:malloc|calloc|realloc)\s*\([^;]*\))\s*;").unwrap_or_else(|e| panic!("regex should compile: {}", e))
});
...
pub static BASE_ARRAY_ACCESS: Lazy<Regex> = Lazy::new(|| {
    Regex::new(r"\*\s*\(\s*(?P<base>[\w\->\.]+)\s*\+\s*(?P<idx>[\w\->\.0-9]+)\s*\)").unwrap_or_else(|e| panic!("regex should compile: {}", e))
});

/// Reverse array access pattern: *(idx + base)
pub static REV_ARRAY_ACCESS: Lazy<Regex> = Lazy::new(|| {
    Regex::new(r"\*\s*\(\s*(?P<idx>\d+|0x[0-9a-fA-F]+)\s*\+\s*(?P<base>[\w\->\.]+)\s*\)").unwrap_or_else(|e| panic!("regex should compile: {}", e))
});
...
pub static WHILE_FALSE: Lazy<Regex> = Lazy::new(|| {
    Regex::new(r"(?s)\bwhile\s*\(\s*(?:false|0)\s*\)\s*\{(?P<body>[^}]*)\}").unwrap_or_else(|e| panic!("regex should compile: {}", e))
});
...
pub static IF_FALSE: Lazy<Regex> = Lazy::new(|| {
    Regex::new(r"(?s)\bif\s*\(\s*(?:false|0)\s*\)\s*\{(?P<body>[^}]*)\}").unwrap_or_else(|e| panic!("regex should compile: {}", e))
});
...
pub static IF_TRUE: Lazy<Regex> = Lazy::new(|| {
    Regex::new(r"(?s)\bif\s*\(\s*(?:true|1)\s*\)\s*\{(?P<body>[^}]*)\}").unwrap_or_else(|e| panic!("regex should compile: {}", e))
});

/// Generic local variable pattern
pub static LOCAL_VAR_PATTERN: Lazy<Regex> =
    Lazy::new(|| Regex::new(r"\b(local_\w+|[a-z]Var\d+)\b").unwrap_or_else(|e| panic!("regex should compile: {}", e)));

/// Local variable assignment pattern
pub static LOCAL_VAR_ASSIGN: Lazy<Regex> =
    Lazy::new(|| Regex::new(r"^\s*(local_\w+|[a-z]Var\d+)\s*=\s*(.+?)\s*;\s*$").unwrap_or_else(|e| panic!("regex should compile: {}", e)));

/// Function call context pattern
pub static FUNC_CALL: Lazy<Regex> = Lazy::new(|| Regex::new(r"\w+\s*\(").unwrap_or_else(|e| panic!("regex should compile: {}", e)));
...
pub static DEFAULT_RETURN: Lazy<Regex> = Lazy::new(|| {
    Regex::new(r"^\s*(return\s+[^;]+;)\s*$").unwrap_or_else(|e| panic!("regex should compile: {}", e))
});
...
pub static ML_NOT_OPEN: Lazy<Regex> =
    Lazy::new(|| Regex::new(r"^(\s*)if\s*\(\s*!(\w+)\s*\)\s*\{\s*$").unwrap_or_else(|e| panic!("regex should compile: {}", e)));
...
pub static IF_NOT_OPEN: Lazy<Regex> = Lazy::new(|| {
    Regex::new(r"^(\s*)if\s*\(\s*!(\w+)\s*\)\s*\{\s*$").unwrap_or_else(|e| panic!("regex should compile: {}", e))
});
...
pub static ELSE_OPEN: Lazy<Regex> = Lazy::new(|| {
    Regex::new(r"^\s*(?:\}\s*)?else\s*\{\s*$").unwrap_or_else(|e| panic!("regex should compile: {}", e))
});

/// Return variable pattern: return var;
pub static RETURN_VAR: Lazy<Regex> = Lazy::new(|| {
    Regex::new(r"^\s*return\s+(\w+)\s*;\s*$").unwrap_or_else(|e| panic!("regex should compile: {}", e))
});
