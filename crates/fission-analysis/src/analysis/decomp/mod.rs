...
fn merge_type_fields(existing: &mut Vec<InferredFieldInfo>, incoming: Vec<InferredFieldInfo>) {
    for field in incoming {
        if let Some(current) = existing.iter_mut().find(|current| current.offset == field.offset) {
...
fn merge_inferred_types(
        ...
        .chain(loader_types.iter().cloned())
    {
        if let Some(existing) = merged.iter_mut().find(|current| {
            ...
            metadata_address != 0
                && incoming_metadata != 0
                && metadata_address == incoming_metadata
...
fn log_type_diag(
    ...
    let count_fields = |items: &[InferredTypeInfo]| -> usize {
        items.iter().map(|ty| ty.fields.len()).sum()
    };
...
impl CachingDecompiler {
    ...
    pub fn decompile(&mut self, address: u64) -> fission_core::Result<String> {
        ...
        log_type_diag(address, &function_types, &self.inferred_types, &merged_types);
