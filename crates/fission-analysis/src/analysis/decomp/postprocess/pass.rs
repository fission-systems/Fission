...
impl PassContext {
    ...
    /// Set inferred types
    pub fn with_inferred_types(mut self, types: Vec<fission_loader::loader::types::InferredTypeInfo>) -> Self {
        self.inferred_types = types;
        self
    }
    
    /// Set DWARF function info
    pub fn with_dwarf_info(mut self, info: Option<fission_loader::loader::types::DwarfFunctionInfo>) -> Self {
...
impl PassRegistry {
    ...
    pub fn execute_all<'a>(&self, code: &'a str, context: &PassContext) -> PassResult<'a> {
        ...
        for pass_id in &self.execution_order {
            ...
            let pass = self.passes.get(pass_id)
                .ok_or_else(|| PassError::ExecutionFailed(
                    pass_id.clone(),
    ...
    fn visit_pass(
        ...
        if visiting.contains(pass_id) {
            return Err(format!("Circular dependency detected involving '{}'", pass_id));
        }
...
mod tests {
    ...
    fn test_pass_registration() {
        ...
        let pass = Box::new(TestPass { id: "test", deps: vec![] });
