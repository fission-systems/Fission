...
fn extract_ascii_strings(data: &[u8], base_addr: u64, min_length: usize) -> Vec<ExtractedString> {
    ...
    for (i, &byte) in data.iter().enumerate() {
        if is_printable_ascii(byte) {
            ...
            if current_string.len() >= min_length
                && let Ok(s) = String::from_utf8(current_string.clone()) {
    ...
    // Check last string
    if current_string.len() >= min_length
        && let Ok(s) = String::from_utf8(current_string) {
            let len = s.len();
            strings.push(ExtractedString {
                address: base_addr + start_offset as u64,
                content: s,
...
fn extract_unicode_strings(data: &[u8], base_addr: u64, min_length: usize) -> Vec<ExtractedString> {
    ...
    while i + 1 < data.len() {
        ...
        if is_printable_unicode(char_val) {
            ...
            if current_string.len() >= min_length
                && let Ok(s) = String::from_utf16(&current_string) {
                    strings.push(ExtractedString {
                        address: base_addr + start_offset as u64,
                        content: s,
...
const fn is_printable_unicode(char_val: u16) -> bool {
