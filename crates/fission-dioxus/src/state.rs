...
impl AppState {
    ...
    pub fn editor_code(&self) -> Option<&str> {
        match self.active_tab {
            ...
            EditorTab::Nir => self.decompiled_nir.as_deref().or(self.decompiled_code.as_deref()),
            EditorTab::Hex => None,
        }
    }
    ...
    pub fn classify_function(info: &fission_loader::loader::FunctionInfo) -> FunctionKind {
        if info.is_import && !info.is_thunk_like {
            FunctionKind::Import { library: info.external_library.clone() }
        } else if info.is_thunk_like {
            FunctionKind::Thunk { target: info.thunk_target }
        } else {
            FunctionKind::Code
        }
    }
...
pub fn fuzzy_score(query: &str, target: &str) -> Option<i32> {
    ...
    for (i, &tc) in tchars.iter().enumerate() {
        if qi < qchars.len() && tc == qchars[qi] {
            ...
            if i == 0 { score += 20; }
            if i > 0 && matches!(tchars[i - 1], '_' | ':' | '.' | ' ') { score += 12; }
            last = i;
            qi += 1;
        }
    }
...
pub fn init_app_state() {
    ...
    state.push_log(LogEntry::info("Open a binary via File \u{2192} Open Binary\u{2026}  or drag and drop a file."));
