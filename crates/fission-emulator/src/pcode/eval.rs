...
impl<'a> Evaluator<'a> {
    ...
    fn write_varnode_shadow(&mut self, vn: &Varnode, node: u32) {
        ...
        for i in 0..vn.size as u64 {
            self.state
                .set_shadow_memory(vn.space_id, vn.offset + i, node);
        }
    }
        ...
        match op.opcode {
            ...
            PcodeOpcode::Load => {
                ...
                if let Some(ptr_node) = addr_node {
                    ...
                    let result_node =
                        crate::pcode::memory_mixin::MemoryMixin::handle_symbolic_read(
                            self.state,
                            self.solver,
                            space_id,
                            ptr_node,
            ...
            PcodeOpcode::IntDiv => {
                let (a, b, out) = self.int_binary(&op)?;
                if b == 0 {
                    tracing::warn!("INT_DIV by zero");
                    self.write_varnode_u64(out, 0)?;
                } else {
                    self.write_varnode_u64(out, a.wrapping_div(b))?;
                }
            }
            PcodeOpcode::IntSDiv => {
                ...
                if b == 0 {
                    tracing::warn!("INT_SDIV by zero");
                    self.write_varnode_u64(out, 0)?;
                } else {
                    self.write_varnode_u64(out, a.wrapping_div(b) as u64)?;
                }
            }
            PcodeOpcode::IntRem => {
                let (a, b, out) = self.int_binary(&op)?;
                if b == 0 {
                    tracing::warn!("INT_REM by zero");
                    self.write_varnode_u64(out, 0)?;
                } else {
                    self.write_varnode_u64(out, a.wrapping_rem(b))?;
                }
            }
            PcodeOpcode::IntSRem => {
                let a_raw = self.read_varnode_u64(&op.inputs[0])?;
                ...
                let b = sign_extend(b_raw, sz);
                if b == 0 {
                    tracing::warn!("INT_SREM by zero");
                    self.write_varnode_u64(out, 0)?;
                } else {
                    self.write_varnode_u64(out, a.wrapping_rem(b) as u64)?;
                }
            }
            ...
            PcodeOpcode::FloatLessEqual => {
                ...
                let o = op
                    .output
                    .as_ref()
                    .expect("FLOAT_LESSEQUAL must have output");
                self.write_varnode_u64(o, bool_u64(a <= b))?;
            }
            ...
            PcodeOpcode::CallOther => {
                ...
                return Ok(StepResult::CallOther {
                    userop_id,
                    input_vals,
                    output_size,
                });
