//! GCC/Itanium C++ LSDA (`.gcc_except_table`) parsing.
//!
//! Each FDE in `.eh_frame` with a "zPLR"-style augmentation carries a
//! pointer to a Language-Specific Data Area (LSDA) in `.gcc_except_table` --
//! the per-function table the C++ personality routine walks at unwind time
//! to find which `catch` clause (if any) matches a thrown exception, and
//! where its landing pad is. `gimli` parses `.eh_frame`'s CIE/FDE structure
//! (and already resolves the LSDA *pointer* via `Fde::lsda()`) but has no
//! knowledge of `.gcc_except_table`'s own format -- it's a separate,
//! GCC-specific ad hoc encoding (documented informally in libgcc's
...
//! output (both a static and a `-fPIE` build, to exercise the absolute and
//! pcrel/indirect pointer-encoding paths) before trusting it -- see the test
//! module.
...
pub struct LsdaTypeEntry {
    ...
    /// a caught type (see `catches_std_runtime_error_via_relocation_symbol`
    /// below). `None` for `catch (...)` (`address` is `0` with no
...
/// wrong address from a guessed base -- none of these appeared in either
/// validation fixture, and `datarel` in particular is an embedded/EABI
/// convention this session had no binary to cross-check against.
