...
#[test]
fn likely_trash_prunes_ecx_if_only_used_in_dead_assignments() {
    // param: ecx (trash candidate)
    // body:
    //   uVar1 = ecx;
    //   uVar2 = uVar1;
    //   return 0;
    let mut func = HirFunction {
        name: "test_trash".to_string(),
            int_param_offsets: Vec::new(),
        params: vec![param_binding("ecx", 32)],
        locals: vec![temp_binding("uVar1", 32), temp_binding("uVar2", 32)],
        return_type: int(32),
        body: vec![
            assign("uVar1", varexpr("ecx")),
            assign("uVar2", varexpr("uVar1")),
            return_expr(const_expr(0, 32)),
        ],
        ..Default::default()
    };
    ...
    // ecx and uVar1/uVar2 should be completely removed, and function should have empty parameter list.
    assert!(!code.contains("ecx"), "expected ecx to be removed: {code}");
    assert!(!code.contains("uVar1"), "expected uVar1 to be removed: {code}");
    assert!(func.params.is_empty(), "expected params to be empty");
}

#[test]
fn likely_trash_preserves_ecx_if_used_in_conditional() {
    // param: ecx
    // body:
    //   if (ecx == 0) { return 1; }
    //   return 0;
    let mut func = HirFunction {
        name: "test_non_trash".to_string(),
            int_param_offsets: Vec::new(),
        params: vec![param_binding("ecx", 32)],
        locals: vec![],
        return_type: int(32),
    ...
    // ecx should NOT be removed!
    assert!(code.contains("ecx"), "expected ecx to be preserved: {code}");
    assert_eq!(func.params.len(), 1, "expected parameter to remain");
}

#[test]
fn likely_trash_prunes_masked_ecx() {
    // param: ecx
    // body:
    //   uVar1 = ecx & 0xffffff00;
    //   uVar2 = uVar1;
    //   return 0;
    let mut func = HirFunction {
        name: "test_masked_trash".to_string(),
            int_param_offsets: Vec::new(),
        params: vec![param_binding("ecx", 32)],
        locals: vec![temp_binding("uVar1", 32), temp_binding("uVar2", 32)],
        return_type: int(32),
        body: vec![
            assign("uVar1", HirExpr::Binary {
                op: HirBinaryOp::And,
                lhs: Box::new(varexpr("ecx")),
                rhs: Box::new(const_expr(0xffffff00u32 as i64, 32)),
                ty: int(32),
            }),
            assign("uVar2", varexpr("uVar1")),
            return_expr(const_expr(0, 32)),
