...
impl<'a> PreviewBuilder<'a> {
    pub(in crate::nir::builder) fn recover_aggregate_store_rhs_from_block(
        &mut self,
        ...
        for _ in 0..4 {
            ...
            for op in block.ops.iter().take(current_scan_end).rev() {
                ...
                let (slot_name, _) = self.ensure_stack_slot_binding(
                    base,
                    offset,
                    NirType::Aggregate { size: reg_vn.size },
                )?;
...
pub(in crate::nir::builder) fn find_prior_def_in_block<'a>(
