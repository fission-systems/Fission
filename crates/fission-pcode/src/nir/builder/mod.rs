        ...
        for _ in 0..8 {
            ...
            if debug {
                ...
                append_preview_debug_trace(&format!(
                    "[mlil-preview][agg] block=0x{:x} op_idx={} current=({},0x{:x},sz={}) scan_end={}",
                    current_block.start_address, op_idx, current.space_id, current.offset, current.size, scan_end
                ));
            }
            if current.space_id == REGISTER_SPACE_ID && current.size >= 16 {
                if let Some((source_block_idx, source, earliest_idx)) =
                    self.recover_wide_register_source_from_linear_chain(
                        current_block_idx,
                        scan_end,
                        &current,
                    )
            ...
            let Some((def_idx, def_op)) =
                ...
                if debug {
                    let line = format!(
                        ...
                        current_block.start_address, scan_end, current.space_id, current.size, current.offset
                    );
                    eprintln!("{line}");
                    append_preview_debug_trace(&line);
        ...
        for depth in 0..4 {
            ...
            if debug {
                let line = format!(
                    ...
                    depth,
                    block.start_address,
                    self.pcode.blocks[pred_idx].start_address
                );
                eprintln!("{line}");
                append_preview_debug_trace(&line);
            }
    ...
    fn asm_guided_xmm_load_source_from_linear_chain(
        ...
        for _ in 0..4 {
            ...
            for op in block.ops.iter().take(current_scan_end).rev() {
                ...
                let (slot_name, _) =
                    self.ensure_stack_slot_binding(base, offset, NirType::Aggregate {
                        size: reg_vn.size,
                    })?;
...
fn asm_loads_xmm_from_stack(asm: &str, xmm_index: u64) -> bool {
    ...
    src.contains('[')
        && (src.contains("RSP") || src.contains("RBP") || src.contains("ESP") || src.contains("EBP"))
}

fn dump_block_register_defs(
    block: &crate::pcode::PcodeBasicBlock,
    label: &str,
    target: &Varnode,
) {
