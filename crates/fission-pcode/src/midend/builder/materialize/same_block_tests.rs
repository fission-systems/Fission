...
fn lzcnt_intrinsic_rhs_is_low_cost_builder_inline_candidate() {
    let expr = PreHirExpr::Call {
        ...
        args: vec![PreHirExpr::Var("tmp_1".to_string())],
        ty: int(32),
    };
...
fn plain_leaf_single_use_inline_can_flow_into_data_consumer() {
    let expr = PreHirExpr::Var("tmp_1".to_string());
...
fn same_block_replacement_keeps_nonleaf_representatives() {
    ...
    assert!(
        !PreviewBuilder::same_block_replacement_requires_stable_representative(&PreHirExpr::Var(
            "tmp_1".to_string()
        ))
    );
}
...
fn alias_unsafe_hazard_prefers_call_between_def_and_use() {
    ...
    let hazard = PreviewBuilder::classify_alias_unsafe_hazard(
        ...
        None,
        &output,
        &PreHirExpr::Var("tmp_1".to_string()),
    );
...
fn single_consumer_call_rhs_proof_marks_preview_unsafe_internal_call() {
    ...
    let rhs = PreHirExpr::Call {
        ...
        args: vec![PreHirExpr::Var("arg_1".to_string())],
        ty: int(32),
    };
...
fn carry_intrinsic_predicate_proof_marks_boolor_branch_chain() {
    ...
    let rhs = PreHirExpr::Call {
        ...
        args: vec![
            PreHirExpr::Var("a".to_string()),
            PreHirExpr::Var("b".to_string()),
        ],
        ty: int(1),
    };
...
fn popcount_consumer_proof_marks_compare_zero_downstream_use() {
    ...
    let proof = builder
        .describe_popcount_consumer_proof(&block, 0, &output, &PreHirExpr::Var("tmp_1".into()))
...
fn popcount_intand_chain_proof_marks_and_one_compare_zero() {
    ...
    let proof = builder
        .describe_popcount_intand_chain_proof(&block, 0, &output, &PreHirExpr::Var("tmp_1".into()))
...
fn parity_chain_proof_marks_popcount_input_role() {
    ...
    let proof = builder
        .describe_parity_chain_proof(&block, 0, &output, &PreHirExpr::Var("tmp_1".into()))
...
fn parity_chain_proof_marks_popcount_result_role() {
    ...
    let rhs = PreHirExpr::Call {
        ...
        args: vec![PreHirExpr::Var("tmp_1".into())],
        ty: int(32),
    };
...
fn parity_chain_proof_rejects_non_one_mask() {
    ...
    let result = builder
        .describe_parity_chain_proof(&block, 0, &input, &PreHirExpr::Var("tmp_1".into()))
        .expect("parity chain result");
...
fn single_consumer_predicate_proof_marks_compare_zero_same_guard() {
    ...
    let proof = PreviewBuilder::describe_single_consumer_predicate_proof(
        ...
        &PreHirExpr::Binary {
            op: PreHirBinaryOp::Eq,
            lhs: Box::new(PreHirExpr::Var("tmp_1".to_string())),
...
fn single_consumer_predicate_proof_marks_negated_flag_same_guard() {
    ...
    let proof = PreviewBuilder::describe_single_consumer_predicate_proof(
        ...
        &PreHirExpr::Unary {
            ...
            expr: Box::new(PreHirExpr::Var("tmp_1".to_string())),
            ty: int(1),
        },
    )
    .expect("single consumer predicate proof");
...
fn arithmetic_predicate_proof_marks_low_bit_and_one() {
    ...
    let proof = PreviewBuilder::describe_arithmetic_predicate_proof(
        ...
        &PreHirExpr::Binary {
            op: PreHirBinaryOp::And,
            lhs: Box::new(PreHirExpr::Var("flag_bits".to_string())),
            rhs: Box::new(PreHirExpr::Const(1, int(32))),
            ty: int(32),
        },
    )
    ...
    let proof = PreviewBuilder::describe_arithmetic_predicate_proof(
        ...
        &PreHirExpr::Binary {
            ...
            lhs: Box::new(PreHirExpr::Binary {
                ...
                lhs: Box::new(PreHirExpr::Var("flags".to_string())),
                rhs: Box::new(PreHirExpr::Const(3, int(32))),
                ty: int(32),
            }),
            rhs: Box::new(PreHirExpr::Const(1, int(32))),
            ty: int(32),
...
fn address_stable_required_base_kind_recognizes_stack_like_names() {
    assert_eq!(
        PreviewBuilder::classify_address_stable_required_base_kind(&PreHirExpr::Var(
            "home_20".to_string()
        )),
        AddressStableRequiredBaseKind::StackRelative
    );
...
fn address_stable_required_expr_kind_recognizes_nested_load() {
    let expr = PreHirExpr::PtrOffset {
        base: Box::new(PreHirExpr::Load {
            ptr: Box::new(PreHirExpr::Var("param_1".to_string())),
            ty: NirType::Unknown,
        }),
        offset: 8,
...
fn address_stable_required_expr_kind_recognizes_nested_call() {
    let expr = PreHirExpr::Binary {
        ...
        rhs: Box::new(PreHirExpr::Const(4, NirType::Unknown)),
        ty: NirType::Unknown,
    };
    assert_eq!(
        PreviewBuilder::classify_address_stable_required_expr_kind(&expr),
...
fn address_stable_required_expr_kind_recognizes_pure_ptr_arithmetic() {
    let expr = PreHirExpr::PtrOffset {
        base: Box::new(PreHirExpr::Binary {
            op: PreHirBinaryOp::Add,
            lhs: Box::new(PreHirExpr::Var("local_10".to_string())),
            rhs: Box::new(PreHirExpr::Const(8, NirType::Unknown)),
            ty: NirType::Unknown,
        }),
        offset: 4,
    };
...
fn stack_address_base_reg_recognizes_rsp() {
    assert_eq!(
        PreviewBuilder::classify_stack_address_base_reg(&PreHirExpr::Var("rsp".to_string())),
        StackAddressBaseReg::Rsp
    );
}
...
fn stack_address_base_reg_recognizes_rbp() {
    let expr = PreHirExpr::PtrOffset {
        base: Box::new(PreHirExpr::Cast {
            ty: test_ptr(),
            expr: Box::new(PreHirExpr::Var("rbp".to_string())),
        }),
        offset: 0x20,
    };
    assert_eq!(
...
fn stack_address_offset_extracts_add_const() {
    let expr = PreHirExpr::Binary {
        op: PreHirBinaryOp::Add,
        lhs: Box::new(PreHirExpr::Var("rsp".to_string())),
        rhs: Box::new(PreHirExpr::Const(0x28, int(64))),
        ty: test_ptr(),
    };
    assert_eq!(
...
fn stack_address_offset_extracts_sub_const() {
    let expr = PreHirExpr::Binary {
        ...
        rhs: Box::new(PreHirExpr::Const(0x18, int(64))),
        ty: test_ptr(),
    };
    assert_eq!(
        PreviewBuilder::extract_stack_address_offset(&expr),
...
fn stack_address_frame_relative_candidate_accepts_ptr_offset() {
    let expr = PreHirExpr::PtrOffset {
        base: Box::new(PreHirExpr::Cast {
            ty: test_ptr(),
            expr: Box::new(PreHirExpr::Var("rsp".to_string())),
        }),
        offset: 0x30,
    };
...
fn stack_address_frame_relative_candidate_rejects_complex_binary() {
    let expr = PreHirExpr::Binary {
        op: PreHirBinaryOp::Add,
        lhs: Box::new(PreHirExpr::Var("rsp".to_string())),
        rhs: Box::new(PreHirExpr::Var("xVar31".to_string())),
        ty: test_ptr(),
    };
    assert!(!PreviewBuilder::stack_address_frame_relative_candidate(
        &expr
