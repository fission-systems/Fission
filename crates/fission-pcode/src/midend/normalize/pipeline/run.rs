...
use crate::midend::vsa::{apply_jump_resolver_pass, jump_resolver_candidate_count};
use std::time::Instant;
use tracing::{debug, debug_span};

pub(crate) fn apply_type_signature_fixed_point(func: &mut HirFunction, diag: bool, perf: bool) {
...
pub(crate) fn cleanup_func_stmt_list(func: &mut HirFunction) {
    ...
    let global_refs = crate::midend::normalize::cleanup::utils::collect_referenced_labels(&func.body);
...
pub(crate) fn body_contains_popcount_call(body: &[HirStmt]) -> bool {
...
pub(crate) fn contains_call_stmts(stmts: &[HirStmt]) -> bool {
...
pub(crate) struct JumpResolverAdmission {
    pub(crate) eligible: bool,
    pub(crate) candidate_scoped: bool,
}

#[derive(Debug, Clone, Copy)]
pub(crate) struct SccpAdmissionSummary {
    pub(crate) eligible: bool,
}

pub(crate) fn jump_resolver_admission(body: &[HirStmt]) -> JumpResolverAdmission {
...
pub(crate) fn sccp_admission_summary(body: &[HirStmt]) -> SccpAdmissionSummary {
...
pub(crate) fn memory_fact_prefilter_allows_full(func: &HirFunction) -> bool {
    func.params
        .iter()
...
pub(crate) fn run_cleanup_family_passes(
    func: &mut HirFunction,
    stage: &str,
    perf: bool,
    budget: PassBudget,
    ...
    if !func.body.is_empty() {
        ...
        if body_has_boundary_label_shapes(&func.body) {
            ...
            changed |= run_pass_logged(
                ...
                |f| {
                    let global_refs =
                        crate::midend::normalize::cleanup::utils::collect_referenced_labels(&f.body);
...
pub(crate) fn body_has_loopish_shapes(stmts: &[HirStmt]) -> bool {
    for stmt in stmts {
        match stmt {
...
fn cleanup_stmt_list(stmts: &mut Vec<HirStmt>, func_name: &str, depth: usize) {
    ...
    let global_refs = if depth == 0 {
        Some(crate::midend::normalize::cleanup::utils::collect_referenced_labels(stmts))
    } else {
        None
    };
    cleanup_stmt_list_with_options_and_preserved(
