...
pub(crate) fn build_normalize_pipeline() -> Pipeline {
...
pub(crate) fn run_normalize_pipeline(func: &mut HirFunction, diag: bool, perf: bool) {
    ...
    if diag {
        eprintln!(
            ...
            func.name,
            crate::midend::action_pipeline::count_hir_stmts(&func.body),
            func.locals.len()
        );
    }
    ...
    if perf {
        let (final_stmts, final_locals) = crate::midend::action_pipeline::hir_shape(func);
        eprintln!(
