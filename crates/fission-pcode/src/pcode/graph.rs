...
impl PcodeGraph {
    ...
    pub fn to_dot(func: &PcodeFunction, def_use: Option<&DefUseTracker>) -> String {
        ...
        for block in &func.blocks {
            ...
            writeln!(dot, "    label=\"Block {} @ 0x{:X}\";", block.index, block.start_address).unwrap();
            ...
            for op in &block.ops {
                ...
                writeln!(dot, "    {} [label=\"{}\", style=filled, fillcolor=\"{}\"];", op_id, label, color).unwrap();
            ...
            for i in 0..block.ops.len().saturating_sub(1) {
                ...
                writeln!(dot, "    op_{}_{} -> op_{}_{} [style=invis];", 
                    block.index, op1.seq_num, block.index, op2.seq_num).unwrap();
            }
                ...
                match last_op.opcode {
                    PcodeOpcode::Branch | PcodeOpcode::CBranch => {
                        ...
                        if !last_op.inputs.is_empty() && last_op.inputs[0].is_constant {
                            ...
                            // Find target block
                            if let Some(target_block) = func.blocks.iter().find(|b| b.start_address == target_addr) {
                                ...
                                if let Some(first_op) = target_block.ops.first() {
                                    let dst_id = format!("op_{}_{}", target_block.index, first_op.seq_num);
                                    let label = if last_op.opcode == PcodeOpcode::CBranch { " [label=\"True\"]" } else { "" };
                                    writeln!(dot, "    {} -> {}{};", src_id, dst_id, label).unwrap();
                                }
                            }
                        }
                        ...
                        if last_op.opcode == PcodeOpcode::CBranch {
                            ...
                            if let Some(next_block) = func.blocks.iter().find(|b| b.index == block.index + 1) {
                                let src_id = format!("op_{}_{}", block.index, last_op.seq_num);
                                if let Some(first_op) = next_block.ops.first() {
                                    let dst_id = format!("op_{}_{}", next_block.index, first_op.seq_num);
                                    writeln!(dot, "    {} -> {} [label=\"False\", style=dashed];", src_id, dst_id).unwrap();
                                }
                            }
                        }
                    }
                    _ => {
                        ...
                        if let Some(next_block) = func.blocks.iter().find(|b| b.index == block.index + 1) {
                            ...
                            if let Some(first_op) = next_block.ops.first() {
                                let dst_id = format!("op_{}_{}", next_block.index, first_op.seq_num);
                                writeln!(dot, "    {} -> {} [style=dashed];", src_id, dst_id).unwrap();
                            }
                        }
                    }
                }
                            ...
                            if let Some(def_op_ref) = tracker.get_def(input) {
                                ...
                                if let Some(def_block) = func.blocks.get(def_op_ref.block_idx) {
                                    if let Some(def_op) = def_block.ops.get(def_op_ref.op_idx) {
                                        let src_id = format!("op_{}_{}", def_block.index, def_op.seq_num);
