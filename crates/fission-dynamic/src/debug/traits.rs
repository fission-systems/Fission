...
pub trait Debugger: Send {
    ...
    fn set_registers(&mut self, thread_id: u32, regs: &RegisterState) -> FissionResult<()> {
        ...
        Err(fission_core::err!(debug, "set_registers is not supported on this platform"))
    }

    /// Launch a new process under the debugger
    fn launch(&mut self, path: &str, args: &[String]) -> FissionResult<u32> {
        let _ = (path, args);
        Err(fission_core::err!(debug, "launch is not supported on this platform"))
    }

    /// Single step over a CALL (step over)
    fn step_over(&mut self) -> FissionResult<()> {
        Err(fission_core::err!(debug, "step_over is not supported on this platform"))
    }

    /// Step out of the current function (run to return)
    fn step_out(&mut self) -> FissionResult<()> {
        Err(fission_core::err!(debug, "step_out is not supported on this platform"))
    }

    /// Pause the target process
    fn pause(&mut self) -> FissionResult<()> {
        Err(fission_core::err!(debug, "pause is not supported on this platform"))
    }

    /// Terminate the target process
    fn terminate(&mut self) -> FissionResult<()> {
        Err(fission_core::err!(debug, "terminate is not supported on this platform"))
    }

    /// Switch the active thread for subsequent operations
    fn set_current_thread(&mut self, thread_id: u32) -> FissionResult<()> {
        let _ = thread_id;
        Err(fission_core::err!(debug, "set_current_thread is not supported on this platform"))
    }

    /// Suspend a thread by ID
    fn suspend_thread(&mut self, thread_id: u32) -> FissionResult<u32> {
        let _ = thread_id;
        Err(fission_core::err!(debug, "suspend_thread is not supported on this platform"))
    }

    /// Resume a thread by ID
    fn resume_thread(&mut self, thread_id: u32) -> FissionResult<u32> {
        let _ = thread_id;
        Err(fission_core::err!(debug, "resume_thread is not supported on this platform"))
    }

    /// Skip the current instruction by advancing the instruction pointer.
    fn skip_instruction(&mut self) -> FissionResult<()> {
        Err(fission_core::err!(debug, "skip_instruction is not supported on this platform"))
    }
    ...
    fn set_memory_breakpoint(
        ...
        Err(fission_core::err!(debug, "set_memory_breakpoint is not supported on this platform"))
    }

    /// Remove a memory breakpoint
    fn remove_memory_breakpoint(&mut self, address: u64) -> FissionResult<()> {
        let _ = address;
        Err(fission_core::err!(debug, "remove_memory_breakpoint is not supported on this platform"))
    }

    /// Set a DLL load breakpoint
    fn set_dll_breakpoint(&mut self, dll_name: &str) -> FissionResult<()> {
        let _ = dll_name;
        Err(fission_core::err!(debug, "set_dll_breakpoint is not supported on this platform"))
    }

    /// Remove a DLL load breakpoint
    fn remove_dll_breakpoint(&mut self, dll_name: &str) -> FissionResult<()> {
        let _ = dll_name;
        Err(fission_core::err!(debug, "remove_dll_breakpoint is not supported on this platform"))
    }

    /// Set an exception breakpoint
    fn set_exception_breakpoint(&mut self, code: u32) -> FissionResult<()> {
        let _ = code;
        Err(fission_core::err!(debug, "set_exception_breakpoint is not supported on this platform"))
    }

    /// Remove an exception breakpoint
    fn remove_exception_breakpoint(&mut self, code: u32) -> FissionResult<()> {
        let _ = code;
        Err(fission_core::err!(debug, "remove_exception_breakpoint is not supported on this platform"))
    }

    /// Enable a breakpoint by address
    fn enable_breakpoint(&mut self, address: u64) -> FissionResult<bool> {
        let _ = address;
        Err(fission_core::err!(debug, "enable_breakpoint is not supported on this platform"))
    }

    /// Disable a breakpoint by address
    fn disable_breakpoint(&mut self, address: u64) -> FissionResult<bool> {
        let _ = address;
        Err(fission_core::err!(debug, "disable_breakpoint is not supported on this platform"))
    }
    ...
    fn remote_alloc(&mut self, address: u64, size: usize) -> FissionResult<u64> {
        let _ = (address, size);
        Err(fission_core::err!(debug, "remote_alloc is not supported on this platform"))
    }

    /// Free memory in the target process
    fn remote_free(&mut self, address: u64) -> FissionResult<()> {
        let _ = address;
        Err(fission_core::err!(debug, "remote_free is not supported on this platform"))
    }

    /// Get page protection rights at an address
    fn get_page_rights(&self, address: u64) -> FissionResult<u32> {
        let _ = address;
        Err(fission_core::err!(debug, "get_page_rights is not supported on this platform"))
    }
    ...
    fn set_page_rights(&mut self, address: u64, size: usize, protect: u32) -> FissionResult<()> {
        ...
        Err(fission_core::err!(debug, "set_page_rights is not supported on this platform"))
    }

    /// Peek a value from the stack at an offset from RSP
    fn stack_peek(&self, offset: isize) -> FissionResult<u64> {
        let _ = offset;
        Err(fission_core::err!(debug, "stack_peek is not supported on this platform"))
    }

    /// Pop a value from the stack (adjusts RSP and returns the value)
    fn stack_pop(&mut self) -> FissionResult<u64> {
        Err(fission_core::err!(debug, "stack_pop is not supported on this platform"))
    }

    /// Push a value onto the stack (adjusts RSP)
    fn stack_push(&mut self, value: u64) -> FissionResult<()> {
        let _ = value;
        Err(fission_core::err!(debug, "stack_push is not supported on this platform"))
    }
    ...
    fn find_pattern(&self, start: u64, size: usize, pattern: &[u8]) -> FissionResult<Vec<u64>> {
        ...
        Err(fission_core::err!(debug, "find_pattern is not supported on this platform"))
    }
    ...
    fn get_module_exports(&self, base: u64) -> FissionResult<Vec<super::types::ExportInfo>> {
        let _ = base;
        Err(fission_core::err!(debug, "get_module_exports is not supported on this platform"))
    }
    ...
    fn get_module_imports(&self, base: u64) -> FissionResult<Vec<super::types::ImportInfo>> {
        let _ = base;
        Err(fission_core::err!(debug, "get_module_imports is not supported on this platform"))
    }
    ...
    fn set_register(&mut self, thread_id: u32, name: &str, value: u64) -> FissionResult<()> {
        ...
        Err(fission_core::err!(debug, "set_register is not supported on this platform"))
    }

    /// Get a CPU flag by name (e.g. "zf", "cf")
    fn get_flag(&self, flag: &str) -> FissionResult<bool> {
        let _ = flag;
        Err(fission_core::err!(debug, "get_flag is not supported on this platform"))
    }

    /// Set a CPU flag by name (e.g. "zf", "cf")
    fn set_flag(&mut self, flag: &str, value: bool) -> FissionResult<()> {
        let _ = (flag, value);
        Err(fission_core::err!(debug, "set_flag is not supported on this platform"))
    }
}
