...
use fission_midend_core::ir::{
    CallEffectSummarySource, NirCallEffectSummary, parse_call_target_address,
};
            ...
            if matches!(
                ...
                if let Some(target) = host.suffix_call_uses_preview_unsafe_callee(stmt) {
                    host.bump_structuring_counter(
                        StructuringCounter::guarded_tail_rejected_side_effectful_callee_count,
                        (1) as usize,
                    );
                    eprintln!(
...
pub fn candidate_window_can_shrink_to_label_with_diag(
    ...
    let suffix_result = suffix_is_nonowned_terminal_tail_with_diag(
        host,
        body,
        anchor_idx,
        candidate_label,
...
pub fn find_earliest_owned_join_label_with_diag(
    ...
    for candidate_label_idx in anchor_idx + 1..terminal_label_idx {
        ...
        let suffix_result = candidate_window_can_shrink_to_label_with_diag(
            host,
            body,
            anchor_idx,
            candidate_label,
