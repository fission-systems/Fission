...
pub fn canonicalize_interleaved_local_aliases(
    ...
    for (idx, stmt) in body.iter().enumerate() {
        ...
        let effective_top_level_after_label_count =
            top_level_after_label_count.saturating_sub(sink_equivalent_top_level_after_label_count);
        let blocking_top_level_after_positions: Vec<usize> = top_level_after_positions
            ...
            .filter(|pos| {
                ...
                ) && !crate::guarded_tail::pure_hir::top_level_after_label_ref_is_dead_post_return(
                    body, *pos, label,
                )
            })
            .collect();
        ...
        if host.guarded_tail_trace_enabled() && sink_equivalent_top_level_after_label_count > 0 {
            eprintln!(
        ...
        if nested_after_label_count > 0
            || (effective_top_level_after_label_count > 0 && !allow_top_level_after_label_redirect)
        {
        ...
        let forward_chain_redirect = if allow_top_level_after_label_redirect
            ...
            let resolved = if next_label_idx.is_some() {
                ...
                crate::guarded_tail::pure_hir::inferred_alias_forward_target_with_after_label_refs(
                    segment, label,
                )
            };
        ...
        let immediate_next_redirect = if forward_chain_redirect.is_none() {
            if let Some(next_label_idx) = next_label_idx {
                if let PreHirStmt::Label(next_label) = &body[next_label_idx] {
                    if crate::guarded_tail::pure_hir::is_local_alias_forward_segment(
                        segment, next_label,
        ...
        if let Some(next_label) = next_redirect_label {
            if external_ref_count > 0 {
                ...
                let nested_before_proof = if external_nested_before > 0 {
                    Some(
                        crate::guarded_tail::pure_hir::build_nested_before_alias_ownership_proof(
                            full_body,
                            segment_start,
                            segment_end,
                            label,
            ...
            if has_non_ignorable_gap {
                if goto_positions.len() != 1
                    && !crate::guarded_tail::pure_hir::is_pure_multi_goto_gap_to_label(
                        body,
                        goto_positions,
                        idx,
                        label,
    ...
    host.bump_structuring_counter(
        StructuringCounter::canonicalized_interleaved_join_use_count,
        (alias_redirects.len()) as usize,
    );
    host.bump_structuring_counter(
        StructuringCounter::canonicalized_local_nonfallthrough_alias_count,
        (canonicalized_local_nonfallthrough) as usize,
    );
        ...
        body.iter()
            .filter_map(|stmt| match stmt {
                PreHirStmt::Goto(label) if alias_redirects.contains_key(label) => {
                    match crate::guarded_tail::pure_hir::resolve_alias_redirect(
                        label,
                        &alias_redirects,
                    ) {
...
pub fn canonicalize_guarded_tail_segment(
    ...
    let collapsed_guards =
        crate::guarded_tail::pure_hir::collapse_duplicate_top_level_guard_ladder(&mut flattened);
    ...
    let collapsed_sink_returns =
        crate::guarded_tail::pure_hir::collapse_top_level_sink_to_return_goto_chain(
            &mut flattened,
            full_body,
        );
    ...
    let (flattened, external_redirects) = canonicalize_interleaved_local_aliases(
        host,
        &flattened[start..end],
        full_body,
        segment_start,
            ...
            PreHirStmt::Label(label) => {
                if referenced.get(label).copied().unwrap_or(0) > 0 {
                    ...
                    let terminalizable_target =
                        crate::guarded_tail::pure_hir::terminalizable_join_alias_target(
                            &flattened, idx,
                        );
                    ...
                    if let Some((next_label, next_idx)) = terminalizable_target {
                        crate::guarded_tail::pure_hir::rewrite_goto_label_in_stmts(
                            &mut canonical,
                            label,
                            &next_label,
                        );
            ...
            PreHirStmt::Goto(_) => {
                if saw_payload {
                    ...
                    if let Some(return_stmt) =
                        crate::guarded_tail::pure_hir::resolve_terminal_tail_exit_stmt(
                            full_body, target,
                        )
                    {
    ...
    if removed_any {
        host.bump_structuring_counter(
            StructuringCounter::canonicalized_guarded_tail_shape_count,
            (1) as usize,
        );
    }
...
pub fn try_build_guarded_tail_witness(
    ...
    let (owned_join_label, label_idx) = host
    ...
    let (middle, external_redirects) = match canonicalize_guarded_tail_segment(
        host,
...
pub fn verify_guarded_tail_trial(
    ...
    host.bump_structuring_counter(
        StructuringCounter::guarded_tail_replacement_plan_candidate_count,
        (1) as usize,
    );
    ...
    let rewritten = crate::guarded_tail::pure_hir::rewrite_guarded_tail_sequence(
        &witness.middle,
        &witness.target_label,
        &[],
    );
    let (outside_refs, middle_refs) =
        crate::guarded_tail::pure_hir::surviving_label_refs_after_guarded_tail_promotion(
            body,
            &rewritten.stmts,
            idx,
            witness.label_idx,
    ...
    let execution_safe = crate::guarded_tail::pure_hir::guarded_tail_middle_is_execution_safe(
        &rewritten.stmts,
        &witness.target_label,
    );
    if crate::guarded_tail::pure_hir::guarded_tail_diag_enabled() {
    ...
    let exported_bindings = match collect_guarded_tail_exported_bindings(
        host,
        &rewritten.stmts,
        follow_tail,
    ) {
    ...
    {
        host.bump_structuring_counter(
            StructuringCounter::guarded_tail_replacement_plan_rejected_missing_merge_count,
            (1) as usize,
        );
        if crate::guarded_tail::pure_hir::guarded_tail_diag_enabled() {
    ...
    if replacement_complete {
        host.bump_structuring_counter(
            StructuringCounter::guarded_tail_replacement_plan_completed_count,
            (1) as usize,
        );
        if crate::guarded_tail::pure_hir::guarded_tail_diag_enabled() {
            eprintln!(
        ...
        host.record_guarded_tail_blockgraph_proof(idx, witness, BlockGraphLegalityReason::Complete);
        return GuardedTailVerification {
            region_legality: legality,
    ...
    if !removable_ops_legal || effective_middle_refs > 0 {
        host.bump_structuring_counter(
            StructuringCounter::guarded_tail_replacement_plan_rejected_unstable_read_count,
            (1) as usize,
        );
    ...
    if host.guarded_tail_trace_enabled() {
        ...
        host.guarded_tail_trace_emit_snapshot("[GT-TRACE] reject_snapshot", &rewritten.stmts, 20);
    }
...
pub fn build_guarded_tail_execution_plan(
    ...
    for binding_idx in 0..exported_bindings.len() {
        ...
        let replacement_target = host.alloc_temp_binding(ty, Some(NirBindingOrigin::TempPreserved));
        host.bump_structuring_counter(
            StructuringCounter::guarded_tail_replacement_plan_merge_created_count,
            (1) as usize,
        );
...
pub fn execute_guarded_tail_plan(
    ...
    for merge in &plan.synthetic_merges {
        for read in &merge.read_sites {
            ...
            if let Some(stmt) = body.get_mut(stmt_idx) {
                ...
                host.bump_structuring_counter(
                    StructuringCounter::guarded_tail_replacement_read_rewritten_count,
                    (1) as usize,
                );
            }
        }
    ...
    host.bump_structuring_counter(
        StructuringCounter::guarded_tail_promoted_count,
        (1) as usize,
    );
...
pub fn discover_guarded_tail_candidates_in_body(
    ...
    for idx in 0..body.len() {
        ...
        host.bump_structuring_counter(
            StructuringCounter::discovery_seen_guarded_tail_like_shape_count,
            (1) as usize,
        );
        ...
        host.bump_structuring_counter(
            StructuringCounter::guarded_tail_candidate_count,
            (1) as usize,
        );
...
pub fn collect_guarded_tail_exported_bindings(
    ...
    for (def_stmt_idx, stmt) in middle.iter().enumerate() {
        ...
        let always_terminates =
            crate::guarded_tail::pure_hir::statement_sequence_always_terminates(middle);
        if crate::guarded_tail::pure_hir::guarded_tail_diag_enabled() {
            eprintln!(
        ...
        host.bump_structuring_counter(
            StructuringCounter::guarded_tail_exported_binding_count,
            (1) as usize,
        );
        host.bump_structuring_counter(
            StructuringCounter::guarded_tail_replacement_read_count,
            (read_sites.len()) as usize,
        );

        if !crate::guarded_tail::pure_hir::expr_is_pure_value(rhs) {
            host.bump_structuring_counter(
                StructuringCounter::guarded_tail_replacement_read_rejected_nonremovable_op_count,
                (read_sites.len()) as usize,
            );
            return Err(GuardedTailExecutionRejection::ReplacementIncomplete);
        }
        ...
        {
            host.bump_structuring_counter(
                StructuringCounter::guarded_tail_replacement_read_rejected_nondominated_count,
                (read_sites.len()) as usize,
            );
            return Err(GuardedTailExecutionRejection::ReplacementIncomplete);
        }
        if nondominated_reads > 0 {
            host.bump_structuring_counter(
                StructuringCounter::guarded_tail_replacement_read_rejected_nondominated_count,
                (read_sites.len() + nondominated_reads) as usize,
            );
            return Err(GuardedTailExecutionRejection::ReplacementIncomplete);
        }
...
pub fn map_guarded_tail_canonicalization_rejection(
