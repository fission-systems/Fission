use crate::cfg_analysis::{PostDomTree, SccAnalysis};
...
pub struct NodeSplitResult {
    pub new_successors: Vec<Vec<usize>>,
    pub new_predecessors: Vec<Vec<usize>>,
    /// For each virtual block (index ≥ original_count), the original block
    /// index whose P-code content it clones.
    pub virtual_to_original: Vec<usize>,
...
impl NodeSplitResult {
    ...
    pub fn original_for(&self, idx: usize) -> usize {
...
pub fn compute_fas_virtual_gotos(
    successors: &[Vec<usize>],
    predecessors: &[Vec<usize>],
) -> Vec<(usize, usize)> {
    ...
    for (component_nodes, _extra_headers) in &irreducible {
        ...
        if node_count <= 64 && candidate_edges.len() <= 16 {
            ...
            for &(src, dst) in &candidate_edges {
                ...
                let postdom = crate::cfg_analysis::PostDomTree::analyze(&temp_succs, &temp_preds);
