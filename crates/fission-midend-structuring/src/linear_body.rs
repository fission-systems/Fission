...
pub fn lower_linear_body(
    host: &mut impl StructuringHost,
    start_idx: usize,
    exit: LinearExit,
) -> Result<Option<(Rc<Vec<PreHirStmt>>, usize)>, MlilPreviewError> {
    lower_linear_body_with_budget(host, start_idx, exit, None)
}

pub fn lower_linear_body_with_budget(
    host: &mut impl StructuringHost,
    start_idx: usize,
    exit: LinearExit,
    ...
    let detailed = lower_linear_body_cached(host, start_idx, exit, 0, Some(budget_ref), false)?;
    Ok(match &detailed {
...
pub fn lower_linear_body_for_region_recovery_detailed(
    host: &mut impl StructuringHost,
    start_idx: usize,
    exit: LinearExit,
    budget: Option<&mut IfLoweringBudget>,
...
pub fn lower_linear_body_cached(
    host: &mut impl StructuringHost,
    start_idx: usize,
    exit: LinearExit,
    depth: usize,
    ...
    let result = lower_linear_body_with_depth_detailed(
        host,
        start_idx,
        exit,
        depth,
...
pub fn lower_linear_body_with_depth_detailed(
    host: &mut impl StructuringHost,
    start_idx: usize,
    exit: LinearExit,
    depth: usize,
    ...
    loop {
        ...
        {
            let tail_lowering = lower_conditional_tail(
                host,
                idx,
                cond,
                true_target,
            ...
            body.push(tail_stmt);
            return Ok(LinearBodyLoweringOutcome::Lowered((Rc::new(body), skip_to)));
        }
        ...
        match terminator {
            LoweredTerminator::Return(expr) => {
                ...
                return Ok(LinearBodyLoweringOutcome::Lowered((Rc::new(body), idx + 1)));
            }
            LoweredTerminator::Fallthrough(Some(target)) | LoweredTerminator::Goto(target) => {
                ...
                if body.is_empty()
                    ...
                    && linear_exit_with_budget(host, next_idx, budget.as_deref_mut())? == Some(exit)
                {
                    return Ok(LinearBodyLoweringOutcome::Lowered((
                        Rc::new(body),
                        next_idx,
...
pub fn shared_exit_for_indices(
    host: &mut impl StructuringHost,
    indices: &[usize],
) -> Result<Option<LinearExit>, MlilPreviewError> {
...
pub fn linear_exit_with_budget(
    host: &mut impl StructuringHost,
    start_idx: usize,
    mut budget: Option<&mut IfLoweringBudget>,
) -> Result<Option<LinearExit>, MlilPreviewError> {
...
pub fn linear_exit_from(
    ...
    match host.lower_block_terminator(idx)? {
        ...
        LoweredTerminator::Cond {
            ...
            let true_exit = linear_exit_from(
                host,
                true_idx,
                &mut true_visited,
                depth + 1,
            ...
            let false_exit = linear_exit_from(
                host,
                false_idx,
                &mut false_visited,
                depth + 1,
...
pub fn can_inline_linear_successor(
    host: &impl StructuringHost,
    idx: usize,
    next_idx: usize,
...
pub fn can_inline_linear_successor_for_region(
    host: &impl StructuringHost,
    idx: usize,
    next_idx: usize,
    visited: &HashSet<usize>,
...
pub fn lower_conditional_tail(
    ...
    let result = (|| {
        ...
        if region_recovery && let LinearExit::Join(join_idx) = exit {
            let shared_tail_entries = match find_shared_tail_entries_for_region(
                host,
                origin_idx,
            ...
            for shared_tail_entry_idx in shared_tail_entries {
                ...
                let true_branch = lower_linear_body_cached(
                    host,
                    true_arm.canonical_idx,
                    shared_exit,
                    depth + 1,
                ...
                let false_branch = lower_linear_body_cached(
                    host,
                    false_arm.canonical_idx,
                    shared_exit,
                    depth + 1,
                ...
                match (true_branch, false_branch) {
                    (
                        ...
                        match lower_linear_body_cached(
                            host,
        ...
        match (true_branch, false_branch) {
            ...
            (LinearBodyLoweringOutcome::Rejected(_), LinearBodyLoweringOutcome::Rejected(_)) => {
...
pub fn normalize_conditional_tail_arm_for_region(
    host: &impl StructuringHost,
    origin_idx: usize,
    start_idx: usize,
    exit: LinearExit,
    ...
    let canonical_idx =
        canonicalize_region_target_for_exit(host, origin_idx, start_idx, exit).unwrap_or(start_idx);
    if let LinearExit::Join(join_idx) = exit {
...
pub fn find_shared_tail_entries_for_region(
    host: &impl StructuringHost,
    origin_idx: usize,
    ...
    let (window, reached_beyond_window) = collect_local_recovery_window_nodes(
        host,
        origin_idx,
        true_start_idx,
        false_start_idx,
...
pub fn collect_local_recovery_window_nodes(
    host: &impl StructuringHost,
    origin_idx: usize,
    true_start_idx: usize,
    false_start_idx: usize,
...
pub fn window_contains_cycle(host: &impl StructuringHost, window: &HashSet<usize>) -> bool {
    ...
    let mut visited = HashSet::default();
    for node in window {
        if !visited.contains(node) && dfs(successors, *node, window, &mut visiting, &mut visited) {
            return true;
        }
    }
    false
...
pub fn compute_local_postdom_sets(
    host: &impl StructuringHost,
    window: &HashSet<usize>,
    join_idx: usize,
...
pub fn shared_follow_candidate_has_side_edge(
    host: &impl StructuringHost,
    origin_idx: usize,
    window: &HashSet<usize>,
    candidate_idx: usize,
...
pub fn collect_region_trivial_forward_chain(
    host: &impl StructuringHost,
    origin_idx: usize,
    start_idx: usize,
    join_idx: usize,
...
pub fn trivial_region_chain_reaches_join(
    host: &impl StructuringHost,
    origin_idx: usize,
    start_idx: usize,
    join_idx: usize,
...
pub fn canonicalize_region_target_for_exit(
    host: &impl StructuringHost,
    origin_idx: usize,
    target_idx: usize,
    exit: LinearExit,
...
pub fn find_shared_tail_entries_for_region_for_test(
    host: &impl StructuringHost,
    origin_idx: usize,
    true_start_idx: usize,
    false_start_idx: usize,
    ...
    match find_shared_tail_entries_for_region(
        host,
        origin_idx,
        true_start_idx,
        false_start_idx,
...
pub fn canonicalize_region_target_for_exit_for_test(
    host: &impl StructuringHost,
    origin_idx: usize,
    target_idx: usize,
    exit: LinearExit,
