...
///
/// The body is deliberately *not* searched for a structurally equal statement
/// sequence. PreHIR statements do not carry CFG block provenance, so equality
/// cannot distinguish an absorbed target block from an unrelated block that
/// happens to contain the same assignment or call. Appending a fresh localized
/// fragment is conservative tail duplication: the orphan edge reaches the
/// target's own terminator, while an existing occurrence remains on its own
/// CFG path. A later owner that preserves block provenance can replace this
/// with label insertion safely; this pass must not guess from syntax alone.
