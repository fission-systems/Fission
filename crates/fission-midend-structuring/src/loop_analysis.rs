...
impl LoopBody {
    ...
    pub fn is_exit(&self, block_idx: usize) -> bool {
    ...
    pub fn preferred_tail_index(&self, successors: &[Vec<usize>]) -> Option<usize> {
    ...
    pub fn order_tails_by_exit(&mut self, successors: &[Vec<usize>]) {
        if self.tails.len() <= 1 {
            return;
        }
