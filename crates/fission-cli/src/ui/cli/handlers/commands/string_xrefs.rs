...
pub fn cmd_string_xrefs(state: &CliState, search_term: &str, min_length: usize) {
    match &state.binary {
        Some(binary) => {
            ...
            println!("{} {}", "Min Length:".dimmed(), min_length.to_string().cyan());
            println!();
            ...
            println!("  Total Strings:       {}", stats.total_strings.to_string().cyan());
            println!("  Referenced:          {} ({}%)", 
            ...
            println!("  Unreferenced:        {}", stats.unreferenced_strings.to_string().dimmed());
            println!("  ASCII:               {}", stats.ascii_strings.to_string().cyan());
            println!("  Unicode:             {}", stats.unicode_strings.to_string().cyan());
            println!("  Total Cross-Refs:    {}", stats.total_xrefs.to_string().green());
            println!();
            ...
            for (i, result) in results.iter().enumerate() {
                if i >= 50 {
                    println!("  {}", format!("... and {} more (showing first 50)", results.len() - 50).dimmed());
                    break;
                }
                ...
                if result.xrefs.is_empty() {
                    ...
                    for (j, xref) in result.xrefs.iter().enumerate() {
                        if j >= 10 {
                            println!("      {}", format!("... and {} more", result.xrefs.len() - 10).dimmed());
                            break;
                        }
