...
use fission_ai::auth::{
    OAuthOptions, ResolvedAuth,
    codex_oauth, copilot_oauth,
    resolve_auth,
...
async fn run_status() -> Result<()> {
    ...
    match &auth {
        ...
        ResolvedAuth::None => {
            ...
            println!("  Run `fission_cli ai login`         for Codex/ChatGPT (ChatGPT Plus required)");
...
async fn run_chat(args: crate::cli::args::AiChatArgs) -> Result<()> {
    ...
    let mut system_prompt = "You are Fission AI, an expert assistant for binary analysis, \
             reverse engineering, and decompilation. Be concise and technically precise.".to_string();
