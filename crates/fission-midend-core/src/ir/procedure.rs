...
/// HirFunction` conversion (`dir_stmts_to_hir_stmts`, in `hir.rs`) produces.
/// `params`/`locals`/`calling_convention`/etc. are shared substrate metadata
/// (declarations and ABI facts that don't change shape across
/// structuring) -- only `body`'s statement grammar (`DirStmt` vs.
/// `HirStmt`) actually differs between [`DirFunction`] and this type. See
/// `hir.rs`'s module doc for why `DirStmt`/`HirStmt` are independently
/// defined rather than one type under two names.
#[derive(Debug, Clone, PartialEq, Eq)]
