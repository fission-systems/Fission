...
fn emit_bcond_predicate(
    ...
    let bool_not =
        |ops: &mut Vec<PcodeOp>, input: Varnode, tag: &str, seq: &mut u32, tmp: &mut u64| {
    ...
    let bool_and = |ops: &mut Vec<PcodeOp>,
                    lhs: Varnode,
                    rhs: Varnode,
                    tag: &str,
                    seq: &mut u32,
    ...
    let bool_or = |ops: &mut Vec<PcodeOp>,
                   lhs: Varnode,
                   rhs: Varnode,
                   tag: &str,
                   seq: &mut u32,
    ...
    let bool_eq = |ops: &mut Vec<PcodeOp>,
                   lhs: Varnode,
                   rhs: Varnode,
                   tag: &str,
                   seq: &mut u32,
    ...
    let bool_ne = |ops: &mut Vec<PcodeOp>,
                   lhs: Varnode,
                   rhs: Varnode,
                   tag: &str,
                   seq: &mut u32,
...
mod tests {
    ...
    fn decode_b_direct_target() {
        ...
        assert_eq!(
            ops[0].inputs[0].constant_val as u64,
            address + ((imm26 as u64) << 2)
        );
    }
    ...
    fn decode_cbz_and_cbnz_predicate_shape() {
        ...
        assert_eq!(
            cbz_ops[1].inputs[0].constant_val as u64,
            address + ((imm19 as u64) << 2)
        );
        ...
        assert_eq!(
            cbnz_ops[1].inputs[0].constant_val as u64,
            address + ((imm19 as u64) << 2)
        );
    ...
    fn decode_tbz_and_tbnz_predicate_shape() {
        ...
        assert_eq!(
            tbz_ops[3].inputs[0].constant_val as u64,
            address + ((imm14 as u64) << 2)
        );
        ...
        assert_eq!(
            tbnz_ops[3].inputs[0].constant_val as u64,
            address + ((imm14 as u64) << 2)
        );
    ...
    fn decode_bcond_gt_le_composed_shapes() {
        ...
        assert!(le_ops[1]
            .inputs
            .iter()
            .any(|vn| vn.space_id == UNIQUE_SPACE_ID && vn.offset == 0xA64F_0001));
        // Z
    }
