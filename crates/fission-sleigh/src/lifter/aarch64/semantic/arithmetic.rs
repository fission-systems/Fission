...
mod tests {
	...
	fn decode_addi_without_flag_writes() {
		...
		let mut seq = 1u32;
		let ops = decode_add_sub_imm(word, 0x1000, &mut temp, &mut seq)
			.expect("expected ADDI decode");

		assert!(ops
			.iter()
			.any(|op| op.opcode == PcodeOpcode::IntAdd && op.asm_mnemonic.as_deref() == Some("ADDI")));
	...
	fn decode_cmp_register_alias_writes_nzcv() {
		...
		let mut temp = A64TempFactory::new(0x1000);
		let mut seq = 1u32;
		let ops = decode_add_sub_reg(word, 0x1000, &mut temp, &mut seq)
			.expect("expected CMP decode");

		assert!(ops
			.iter()
			.any(|op| op.opcode == PcodeOpcode::IntSub && op.asm_mnemonic.as_deref() == Some("CMP")));
