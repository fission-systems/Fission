pub(super) fn compiled_space(name: &str, index: u64) -> CompiledSpaceRef {
    CompiledSpaceRef {
        ...
        sleigh_is_ram_class: name == "ram",
        sleigh_is_unique_space: name == "unique",
    }
}

pub(super) fn fixed_const_handle(value: u64, size: u32) -> RuntimeFixedHandle {
    RuntimeFixedHandle {
...
pub(super) fn bound_operand_from_fixed_handle(handle: &RuntimeFixedHandle) -> Result<BoundOperand> {
    ...
    Ok(BoundOperand::NamedVarnode {
        name: format!("{}_{:x}", space.name, handle.offset_offset),
        display_index: None,
        size: handle.size,
    })
}
