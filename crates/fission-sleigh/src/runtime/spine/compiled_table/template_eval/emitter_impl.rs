impl<'c> CompiledTableEmitter<'c> {
    ...
    /// Returns true when `space` is a plausible **primary** store space for Ghidra-style
    /// dynamic memory handles (LOAD/STORE through a pointer in `offset_space`).
    ///
    /// Historically this path used magic checks (`name == "ram"` or a fixed index). That
    /// breaks across languages where RAM uses another index. Instead we exclude only spaces
    /// that are never primary stores in normal Sleigh models: const, the SLA unique space,
    /// and (when known) the SLA register space — matching the plan’s “SLA metadata only”
    /// direction without hard-coding Ghidra’s typical RAM index (`0x1b1`).
    fn space_is_dynamic_store_primary_space(&self, space: &CompiledSpaceRef) -> bool {
        if space.index == 0 || space.name == "const" {
            return false;
        }
        if space.index == self.compiled.sla_unique_space_index {
            return false;
        }
        if self.compiled.sla_register_space_index != u64::MAX
            && space.index == self.compiled.sla_register_space_index
        {
            return false;
        }
        true
    ...
    fn dynamic_memory_target(
        ...
        if !self.space_is_dynamic_store_primary_space(primary) {
            return Ok(None);
        }
        ...
        Ok(Some(DynamicMemoryTarget {
            space: Varnode::constant(primary.index as i64, 4),
            ptr,
            temp: Varnode {
