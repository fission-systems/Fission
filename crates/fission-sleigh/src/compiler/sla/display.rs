...
fn decode_display_symbols(
    ...
    for symbol in root.descendants_with_id(sla_format::ELEM_VARNODE_SYM) {
        if let Some(id) = symbol.attr_unsigned(sla_format::ATTR_ID).map(|value| value as u32) {
    ...
    for subtable in root.descendants_with_id(sla_format::ELEM_SUBTABLE_SYM) {
        if let Some(id) = subtable.attr_unsigned(sla_format::ATTR_ID).map(|value| value as u32) {
    ...
    for symbol in root.descendants_with_id(sla_format::ELEM_NAME_SYM) {
        if let Some(id) = symbol.attr_unsigned(sla_format::ATTR_ID).map(|value| value as u32) {
            let token_field = symbol.children.first().map(decode_token_field_if_direct).transpose()?.flatten();
            let names = symbol
                .children
                .iter()
    ...
    for symbol in root.descendants_with_id(sla_format::ELEM_VALUEMAP_SYM) {
        if let Some(id) = symbol.attr_unsigned(sla_format::ATTR_ID).map(|value| value as u32) {
            ...
            let values = symbol
                ...
                .collect::<Vec<_>>();
            out.insert(id, DecodedDisplaySymbol::ValueMap { values, token_field });
        }
    }
    for symbol in root.descendants_with_id(sla_format::ELEM_VARLIST_SYM) {
        if let Some(id) = symbol.attr_unsigned(sla_format::ATTR_ID).map(|value| value as u32) {
            ...
            let entries = symbol
                ...
                .collect::<Vec<_>>();
            out.insert(id, DecodedDisplaySymbol::VarnodeList { entries, token_field });
        }
    }
    Ok(out)
}

fn decoded_display_kind(symbol: &DecodedDisplaySymbol) -> CompiledDisplayOperandKind {
    match symbol {
        ...
        DecodedDisplaySymbol::NameTable { names, .. } => CompiledDisplayOperandKind::NameTable(names.clone()),
        DecodedDisplaySymbol::ValueMap { values, .. } => CompiledDisplayOperandKind::ValueMap(values.clone()),
        DecodedDisplaySymbol::VarnodeList { entries, .. } => {
            CompiledDisplayOperandKind::VarnodeList(entries.iter().map(|entry| entry.name.clone()).collect())
        }
        DecodedDisplaySymbol::FixedVarnode(_) => CompiledDisplayOperandKind::Generic,
    }
}

fn decode_token_field_if_direct(element: &PackedElement) -> Result<Option<DecodedTokenField>> {
...
fn operand_piece_label(index: usize) -> char {
