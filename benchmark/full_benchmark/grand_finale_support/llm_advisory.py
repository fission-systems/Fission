...
from .compact_summary import COMPACT_SUMMARY_FILENAME
...
def maybe_generate_benchmark_llm_advisory(
    *,
    ...
    try:
        compact_summary_path = output_dir / COMPACT_SUMMARY_FILENAME
        used_compact_summary = compact_summary_path.is_file()
        if used_compact_summary:
            llm_input = _load_json(compact_summary_path)
            metadata["compact_summary_source"] = str(compact_summary_path.resolve())
        ...
        metadata["same_axis_comparable"] = llm_input.get(
            "same_axis_comparable",
            ((llm_input.get("policy_context") or {}).get("same_axis_comparable")),
        )
