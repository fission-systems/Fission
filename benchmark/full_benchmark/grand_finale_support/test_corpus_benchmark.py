...
from grand_finale_support.benchmark_core import (
    ...
    _build_baseline_regression_report,
...
class CorpusBenchmarkTests(unittest.TestCase):
    def test_baseline_gate_waives_generic_params_for_non_degrading_formal_recovery(self) -> None:
        baseline = _minimal_single_binary_summary(
            avg_similarity=40.0,
            generic_param_name_sum=0,
        )
        current = _minimal_single_binary_summary(
            avg_similarity=41.0,
            generic_param_name_sum=2,
        )
        baseline["pairwise"]["pyghidra_vs_fission"]["comparisons"] = [
            {
                ...
                "fission_generic_param_name_sum": 0,
            }
        ]
        current["pairwise"]["pyghidra_vs_fission"]["comparisons"] = [
            {
                ...
                "fission_generic_param_name_sum": 2,
            }
        ]
        ...
        self.assertIn("shape_generic_param_name_sum", report["waived_regressions"][0])

    def test_baseline_gate_keeps_generic_param_regression_when_row_degrades(self) -> None:
        baseline = _minimal_single_binary_summary(
            avg_similarity=40.0,
            generic_param_name_sum=0,
        )
